object Impressao_FinanceirosOP_Endividamento: TImpressao_FinanceirosOP_Endividamento
  Left = 536
  Top = 322
  BorderStyle = bsDialog
  Caption = 'Impressao_FinanceirosOP_Endividamento'
  ClientHeight = 309
  ClientWidth = 513
  Color = clBtnFace
  ...
  object RxLabel2: TRxLabel
    ...
    Width = 193
    Height = 18
    Caption = 'Empr'#233'stimos (Endividamento).'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWhite
    Font.Height = -15
    Font.Name = 'Calibri'
  ...
  object cEmprestimo: TRadioGroup
    Left = 12
    Top = 137
    Width = 165
    Height = 101
    Caption = 'Tipo de Empr'#233'stimo'
    ItemIndex = 3
    Items.Strings = (
      'Capital de Giro'
  ...
  object rEmprestimosDD: TppReport
    ...
    PrinterSetup.DocumentName = 'Empr'#233'stimos (Endividamento)'
  ...
  object tEmprestimos: TMSQuery
    ...
    SQL.Strings = (
      'SELECT Emprestimo      = Numero_Documento,'
      '       Data,'
      
        '       Banco           = (SELECT Nome FROM Bancos WHERE Codigo =' +
        ' Banco),'
      
        '       Modalidade      = CASE WHEN Tipo = '#39'DD'#39' THEN '#39'Desconto Du' +
        'plicata'#39' '
      
        '                              WHEN Tipo = '#39'CG'#39' THEN '#39'Capital Gir' +
        'o'#39' '
      '                              WHEN Tipo = '#39'FI'#39' THEN '#39'FINIMP'#39' '
      '                         END,'
      '       Garantia,'
      '       Valor_Tomado    = Valor_Principal,'
      
        '       Valor_Pago      = ISNULL((SELECT SUM(Valor) FROM PagarRec' +
        'eberBaixas WHERE Numero = Financeiro_Numero), 0),'
      
        '       Valor_Aberto    = Valor_Principal - ISNULL((SELECT SUM(Va' +
        'lor) FROM PagarReceberBaixas WHERE Numero = Financeiro_Numero), ' +
        '0),'
      
        '       Ultima_Data     = (SELECT MAX(Data) FROM PagarReceberBaix' +
        'as WHERE Numero = Financeiro_Numero),'
      '       Valor_Aplicacao = ISNULL(Valor_Aplicacao, 0),'
      
        '       Valor_Risco     = (Valor_Principal - ISNULL((SELECT SUM(V' +
        'alor) FROM PagarReceberBaixas WHERE Numero = Financeiro_Numero),' +
        ' 0)) - ISNULL(Valor_Aplicacao, 0),'
      
        '       Cliente         = (SELECT Nome FROM Clientes WHERE Codigo' +
        ' = Cliente),'
      '       Emprestimo_Numero = 0,'
      '       Parcelas_Abertas = 0,'
      '       Origem           = '#39'F1'#39
