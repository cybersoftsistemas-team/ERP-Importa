...
begin
      ...
      With dmFiscal, Dados do begin
           ...
           tTotaliza.SQL.Add('       Valor_BCIBS          = (SELECT SUM(ISNULL(Valor_BCIBS, 0))       FROM NotasTerceirosItens NTI WHERE NTI.Nota = NotasTerceiros.Nota AND NTI.Data_Emissao = NotasTerceiros.Data_Emissao AND NTI.Fornecedor = NotasTerceiros.Fornecedor),');
           tTotaliza.SQL.Add('       Valor_IBSMun         = (SELECT SUM(ISNULL(Valor_IBSMun, 0))      FROM NotasTerceirosItens NTI WHERE NTI.Nota = NotasTerceiros.Nota AND NTI.Data_Emissao = NotasTerceiros.Data_Emissao AND NTI.Fornecedor = NotasTerceiros.Fornecedor),');
           tTotaliza.SQL.Add('       Valor_IBSUF          = (SELECT SUM(ISNULL(Valor_IBSUF, 0))       FROM NotasTerceirosItens NTI WHERE NTI.Nota = NotasTerceiros.Nota AND NTI.Data_Emissao = NotasTerceiros.Data_Emissao AND NTI.Fornecedor = NotasTerceiros.Fornecedor),');
           tTotaliza.SQL.Add('       Valor_BCCBS          = (SELECT SUM(ISNULL(Valor_BCCBS, 0))       FROM NotasTerceirosItens NTI WHERE NTI.Nota = NotasTerceiros.Nota AND NTI.Data_Emissao = NotasTerceiros.Data_Emissao AND NTI.Fornecedor = NotasTerceiros.Fornecedor),');
           tTotaliza.SQL.Add('       Valor_CBS            = (SELECT SUM(ISNULL(Valor_CBS, 0))         FROM NotasTerceirosItens NTI WHERE NTI.Nota = NotasTerceiros.Nota AND NTI.Data_Emissao = NotasTerceiros.Data_Emissao AND NTI.Fornecedor = NotasTerceiros.Fornecedor)');
