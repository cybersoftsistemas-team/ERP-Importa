object Impressao_FaturamentoOP_EstoqueResumido: TImpressao_FaturamentoOP_EstoqueResumido
  Left = 513
  Top = 343
  BorderStyle = bsDialog
  Caption = 'Impressao_FaturamentoOP_EstoqueResumido'
  ClientHeight = 268
  ClientWidth = 813
  Color = clBtnFace
  ...
  object RxLabel2: TRxLabel
    ...
    Width = 257
    Height = 18
    Caption = 'Posi'#231#227'o de estoque resumido p/Processo.'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWhite
    Font.Height = -15
    Font.Name = 'Calibri'
  ...
  object cSaldo: TCheckBox
    ...
    Width = 201
    Height = 17
    Caption = 'N'#227'o mostrar Produtos com saldo Zero'
    Checked = True
    State = cbChecked
    TabOrder = 11
  end
  object rEstoqueResumido: TppReport
    AutoStop = False
    DataPipeline = pItens
    NoDataBehaviors = [ndBlankReport]
    PassSetting = psTwoPass
    PrinterSetup.BinName = 'Default'
    PrinterSetup.DocumentName = 'Posi'#231#227'o de Estoque Resumido /Processo'
  ...
  object pItens: TppDBPipeline
    ...
    object pItensppField6: TppField
      Alignment = taRightJustify
      FieldAlias = 'Unidade_Volumes'
      FieldName = 'Unidade_Volumes'
      FieldLength = 0
      DataType = dtDouble
      DisplayWidth = 10
      Position = 5
  ...
  object tItens: TMSQuery
    ...
    SQL.Strings = (
      'SELECT NI.Codigo_Mercadoria AS Codigo,'
      '       NI.Processo'
      'INTO   #Temp'
      'FROM   NotasItens NI'
      'WHERE  (NI.Cancelada <> 1) AND (NI.Movimenta_Estoque = 1)'
      '       AND (NI.Data <= '#39'01/31/2012'#39')'
      'GROUP BY Codigo_Mercadoria, Processo'
      'SELECT Codigo,'
      
        '       Codigo_Fabricante = (SELECT Codigo_Fabricante FROM Produt' +
        'os WHERE(Produtos.Codigo = #Temp.Codigo)),'
      '       Processo,'
      
        '       Descricao       = (SELECT Descricao FROM Produtos WHERE(P' +
        'rodutos.Codigo = #Temp.Codigo)),'
      
        '       NCM             = (SELECT NCM FROM Produtos WHERE(Produto' +
        's.Codigo = #Temp.Codigo)),'
      
        '       Unidade_Volumes = (SELECT Quantidade_Volumes FROM Produto' +
        's WHERE(Produtos.Codigo = #Temp.Codigo)),'
      
        '       Unidade_Medida  = (SELECT Unidade_Origem FROM Produtos WH' +
        'ERE(Produtos.Codigo = #Temp.Codigo)),'
      
        '       Quantidade_Volumes = (SELECT Quantidade_Volumes FROM Prod' +
        'utos WHERE(Produtos.Codigo = #Temp.Codigo)),'
      
        '       Saldo           = ISNULL((SELECT SUM(Quantidade) FROM Not' +
        'asItens NI WHERE(NI.Codigo_Mercadoria = #Temp.Codigo) AND (NI.Sa' +
        'ida_Entrada = 0) AND (NI.Processo = #Temp.Processo) AND (NI.Canc' +
        'elada <> 1)'
      
        '                                 AND (NI.Movimenta_Estoque = 1) ' +
        'AND (Finalidade_Mercadoria IN(0,2)))-'
      
        '                         (SELECT SUM(Quantidade) FROM NotasItens' +
        ' NI WHERE(NI.Codigo_Mercadoria = #Temp.Codigo) AND (NI.Saida_Ent' +
        'rada = 1) AND (NI.Processo = #Temp.Processo) AND (NI.Cancelada <' +
        '> 1)'
      
        '                                 AND (NI.Movimenta_Estoque = 1) ' +
        'AND (Finalidade_Mercadoria IN(0,2))),0),'
      '       Volumes         =  0'
      'INTO #Temp2'
      'FROM #Temp'
      'UPDATE #Temp2 SET Volumes = Saldo / Unidade_Volumes'
      ''
      'SELECT * FROM #Temp2'
      'ORDER BY Codigo,Processo'
