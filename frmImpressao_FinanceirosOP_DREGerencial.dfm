object Impressao_FinanceirosOP_DREGerencial: TImpressao_FinanceirosOP_DREGerencial
  Left = 706
  Top = 198
  BorderStyle = bsDialog
  Caption = 'Impressao_FinanceirosOP_DREGerencial'
  ClientHeight = 383
  ClientWidth = 437
  Color = clBtnFace
  ...
  object cConsolidar: TRadioGroup
    Left = 14
    Top = 64
    Width = 411
    Height = 45
  ...
  object tMovimento: TMSQuery
    ...
    SQL.Strings = (
      ...
      #9'     ,Grupo'
      'INTO #TMP_DRE'
      'FROM Cybersoft_Cadastros.dbo.DRE'
      ...
      '      ,Descricao'
      'INTO #TMP_Grupos'
      'FROM Cybersoft_Cadastros.dbo.DREGrupos'
      'SELECT Codigo'
      '      ,Descricao'
      '      ,Valor = ISNULL('
      
        #9'                  (SELECT SUM( CASE WHEN PR.Tipo = '#39'R'#39' THEN Val' +
        'or_Parcela ELSE Valor_Parcela *-1 END )'
      
        #9'                   FROM PagarReceber PR WHERE PR.Data_Documento' +
        ' BETWEEN '#39'01/08/2016'#39' AND '#39'31/08/2016'#39
      
        #9'                   AND (SELECT DRE_Principal FROM Classificacao' +
        'Financeira CF WHERE CF.Codigo = PR.Classificacao) = Codigo'
      '                    AND ISNULL(PR.Provisorio, 0) <> 1'
      '                    AND (ISNULL(PR.Classificacao, '#39#39') <> '#39#39') )'
      #9#9#9'   ,0)'
      #9'     ,Grupo'
      'INTO #DRE'
      'FROM #TMP_DRE'
      ...
      'GROUP BY Codigo, Descricao, Grupo'
        ...
        '-------------------------------------------- TOTALIZA OS GRUPOS ' +
        'COM BASE NAS DRE'#180's ---------------------------------------'
      'SELECT Codigo'
      '      ,Descricao'
      
        #9'  ,Valor = ISNULL((SELECT SUM(Valor) FROM #DRE_Total WHERE #DRE' +
        '_Total.Grupo = #TMP_Grupos.Codigo), 0)'
      'INTO #Grupos_Total'
      'FROM #TMP_Grupos'
      ...
      #9'  ,Grupo = Codigo'
      '      ,Ordem = '#39'B'#39' '
      'FROM  #Grupos_Total GT'
      ''
      'SELECT Descricao'
      #9'  ,Valor = CASE WHEN Ordem = '#39'A'#39' THEN '
      #9'                Valor '
      #9#9#9'   ELSE '
      
        #9#9#9'       Valor + ISNULL((SELECT SUM(Valor) FROM #Total TT2 WHER' +
        'E TT2.Grupo < #Total.Grupo AND Ordem = '#39'B'#39'), 0)'
      #9#9#9'   END'
      #9'  ,Grupo'
      #9'  ,Ordem'
      'FROM  #Total'
  ...
  object rDRE: TppReport
    ...
    PrinterSetup.DocumentName = 'DRE Gerencial'
    ...
    Template.FileName = 'C:\Sistemas\Faturamento\Codigo_Fonte\Financeiro_DREGerencial.rtm'
    ...
    object ppDetailBand1: TppDetailBand
      ...
      Background1.Gradient.EndColor = 14653184
      Background1.Gradient.StartColor = 16053248
      Background1.Gradient.Style = gsVertical
      Background2.Gradient.EndColor = 16053248
      Background2.Gradient.StartColor = 14653184
      Background2.Gradient.Style = gsVertical
      Border.mmPadding = 0
      mmBottomOffset = 0
      mmHeight = 7408
      mmPrintPosition = 0
      object pValor: TppDBText
        DesignLayer = ppDesignLayer1
        UserName = 'DBText101'
        Border.mmPadding = 0
        DataField = 'Valor'
