unit frmPedidosRepresentantes_PedidoNF;
...
type
  TPedidosRepresentantes_PedidoNF = class(TForm)
    Panel1: TPanel;
    bSair: TButton;
    bGerar: TButton;
    tPedidosNF: TMSQuery;
    ...
    DBGrid2: TDBGrid;
    bRemover: TButton;
    cAdicionados: TCurrencyEdit;
    StaticText7: TStaticText;
    cQtdeDiv: TCurrencyEdit;
    bDividir: TButton;
    bRemTudo: TButton;
    Panel3: TPanel;
  ...
  private
    ...
    procedure SalvaNavio(Quantidade: real; Navio: integer; DI, BL: string);
...
PedidosRepresentantes_PedidoNF: TPedidosRepresentantes_PedidoNF;
...
procedure TPedidosRepresentantes_PedidoNF.bDividirClick(Sender: TObject);
var
 parte: integer;
begin
     with tQuebras do begin
          EmptyDataSet;
          for parte := 1 to cQtdeDiv.asinteger do begin
              append;
                   fieldbyname('Quantidade').Value := cDisponivel.value / cQtdeDiv.AsInteger;
              post;
              cAdicionados.value := cDisponivel.value;
              cSaldo.value       := 0;
              cQtde.value        := 0;
          end;
     end;
...
procedure TPedidosRepresentantes_PedidoNF.bGerarClick(Sender: TObject);
var
  mSaldoDI: real;
...
begin
     ...
     if tQuebras.RecordCount > 0 then begin
        if cAdicionados.value > cTotal.value then begin
           MessageDlg('Não existe estoque suficiente para gerar o pedido de nota.', mtError, [mbOK], 0);
           Abort;
        end;
     end else begin
        if cDisponivel.value > cTotal.value then begin
           MessageDlg('Não existe estoque suficiente para gerar o pedido de nota.', mtError, [mbOK], 0);
           Abort;
        end;
     end;
     ...
     with Dados, dmFiscal do begin
          ...
          with tPedidosNF do begin
               ...
               LibTot := true;
               if tQuebras.RecordCount > 1 then LibTot := false;
               if (tQuebras.RecordCount = 1) and (cSaldo.value > 0) then LibTot := false;

               if not LibTot then begin 
                  CopiaPedidos;
               end;
               ...
               while not eof do begin
                     ...
                     with PedidosItensNavios do begin
                          sql.clear;
                          sql.add('select * from PedidosItensNavios where Pedido = '+Pedidos.FieldByName('Numero').AsString);
                          open;
                     end;
                     with PedidosItensNaviosRemessa do begin
                          sql.clear;
                          sql.add('select * from PedidosItensNaviosRemessa where Pedido = '+Pedidos.FieldByName('Numero').AsString);
                          open;
                     end;
                     ...
                     with tTemp do begin
                          sql.clear;
                          sql.Add('delete from PedidosItensNavios where Pedido = '+PedidosNumero.asstring);
                          execute;
                     end;
                     with tPedidosNFItens do begin
                          ...
                          while not Eof do begin
                                ...
                                // Adiciona os navios com suas quantidades na tabela de Itens de Pedidos de navios.
                                with tMenor do begin
                                     close;
                                     if PedidosItens.FieldByName('Movimenta_Estoque').asboolean then begin
                                        sql.clear;
                                        sql.add('select distinct Navio');
                                        sql.add('               ,Navio_Nome = (select Navio from Cybersoft_Cadastros.dbo.ControleNavios cn where cn.Ordem = nv.Navio)');
                                        sql.add('               ,DI');
                                        sql.add('               ,BL');
                                        sql.add('               ,Disponivel = isnull((select sum(iif(nin.Saida_Entrada = 0, Quantidade, Quantidade*-1)) from NotasItensNavios nin where nin.Navio = nv.Navio and nin.DI = nv.DI), 0)+');
                                        sql.add('                             isnull((select sum(iif(pin.Saida_Entrada = 0, Quantidade, Quantidade*-1)) from PedidosItensNavios pin where pin.Navio = nv.Navio and pin.DI = nv.DI), 0)');
                                        sql.add('               ,Data = (select cast(isnull(min(Data), getdate()) as date) from NotasItens ni where ni.Navio = nv.Navio and ni.Codigo_Mercadoria = nv.Codigo_Mercadoria and ni.Saida_Entrada = 1 and isnull(Cancelada, 0) = 0)');
                                        sql.add('               ,Nota = (select isnull(min(Nota), 0) from NotasItens ni where (ni.Navio = nv.Navio or ni.DI = nv.DI) and ni.Codigo_Mercadoria = nv.Codigo_Mercadoria and ni.Saida_Entrada = 1 and isnull(Cancelada, 0) = 0)');
                                        sql.add('into #tmp');
                                        sql.add('from NotasItensNavios nv');
                                        sql.add('where Codigo_Mercadoria in(select Codigo_Mercadoria from PedidosRepresentantesItens where Pedido in('+mPedidos+'))');
                                        sql.add('and Armazem = :pArmazem');
                                        sql.add('and (isnull((select sum(iif(nin.Saida_Entrada = 0, Quantidade, Quantidade*-1)) from NotasItensNavios nin where nin.Navio = nv.Navio and nin.DI = nv.DI), 0)+');
                                        sql.add('     isnull((select sum(iif(pin.Saida_Entrada = 0, Quantidade, Quantidade*-1)) from PedidosItensNavios pin where pin.Navio = nv.Navio and pin.DI = nv.DI), 0)) > 0');
                                        sql.add('and (select count(*) from NotasItens ni where ni.Navio = nv.Navio and ni.DI = nv.DI and (select isnull(Remessa, 0) from NotasFiscais nf where nf.Numero = ni.Nota and nf.Data_Emissao = ni.Data) = 1');
                                        sql.add('                                        and isnull(Cancelada, 0) = 0 and isnull(NFe_Denegada, 0) = 0) > 0');
                                        sql.add('select top 1 * from #tmp order by Data, Nota, DI');
                                        sql.add('drop table #tmp');
                                        //sql.savetofile('c:\temp\Pedidos_Representantes_NF_Menor.sql');
                                     end else begin
                                        sql.clear;
                                        sql.add('select distinct Navio');
                                        sql.add('               ,Navio_Nome = (select Navio from Cybersoft_Cadastros.dbo.ControleNavios cn where cn.Ordem = nv.Navio)');
                                        ...
                                        sql.add('               ,BL');
                                        sql.add('               ,Disponivel = isnull((select sum(iif(nin.Saida_Entrada = 0, Quantidade, Quantidade*-1)) from NotasItensNavios nin where nin.Navio = nv.Navio and nin.DI = nv.DI), 0)+');
                                        sql.add('                             isnull((select sum(iif(pin.Saida_Entrada = 0, Quantidade, Quantidade*-1)) from PedidosItensNaviosRemessa pin where pin.Navio = nv.Navio and pin.DI = nv.DI), 0)');
                                        sql.add('               ,Data = (select cast(isnull(min(Data), getdate()) as date) from NotasItens ni where ni.Navio = nv.Navio and ni.Codigo_Mercadoria = nv.Codigo_Mercadoria and ni.Saida_Entrada = 1 and isnull(Cancelada, 0) = 0)');
                                        sql.add('               ,Nota = (select isnull(min(Nota), 0) from NotasItens ni where (ni.Navio = nv.Navio or ni.DI = nv.DI) and ni.Codigo_Mercadoria = nv.Codigo_Mercadoria and ni.Saida_Entrada = 1 and isnull(Cancelada, 0) = 0)');
                                        sql.add('into #tmp');
                                        sql.add('from NotasItensNavios nv');
                                        ...
                                        sql.add('and Armazem = :pArmazem');
                                        sql.add('and (isnull((select sum(iif(nin.Saida_Entrada = 0, Quantidade, Quantidade*-1)) from NotasItensNavios nin where nin.Navio = nv.Navio and nin.DI = nv.DI), 0)+');
                                        sql.add('     isnull((select sum(iif(pin.Saida_Entrada = 0, Quantidade, Quantidade*-1)) from PedidosItensNaviosRemessa pin where pin.Navio = nv.Navio and pin.DI = nv.DI), 0)) > 0');
                                        sql.add('and (select count(*) from NotasItens ni where ni.Navio = nv.Navio and ni.DI = nv.DI and (select isnull(Remessa, 0) from NotasFiscais nf where nf.Numero = ni.Nota and nf.Data_Emissao = ni.Data) = 1');
                                        sql.add('                                        and isnull(Cancelada, 0) = 0 and isnull(NFe_Denegada, 0) = 0) > 0');
                                        sql.add('select top 1 * from #tmp order by Data, Nota, DI');
                                        sql.add('drop table #tmp');
                                        //sql.savetofile('c:\temp\Pedidos_Representantes_NF_Menor_Remessa.sql');
                                     end;
                                     parambyname('pArmazem').AsInteger := Pedidos.FieldByName('Armazem').AsInteger;

                                     mSaldoDI := PedidosItens.FieldByName('Quantidade').asfloat;
                                     while mSaldoDI > 0 do begin
                                           // Pegando a DI com a menor quantidade disponivel.
                                           open;
                                           ...
                                           // Salva o navio.
                                           if fieldbyname('Disponivel').asfloat >= mSaldoDI then begin
                                              SalvaNavio(mSaldoDI, fieldbyname('Navio').asinteger, fieldbyname('DI').asstring, fieldbyname('BL').asstring);
                                              mSaldoDI := 0;
                                           end else begin
                                              SalvaNavio(FieldByName('Disponivel').asfloat, fieldbyname('Navio').asinteger, fieldbyname('DI').asstring, fieldbyname('DI').asstring);
                                              mSaldoDI := mSaldoDI - fieldbyname('Disponivel').asfloat;
                                           end;
                                           close;
                                     end;
                                end;
                     ...
                     Pedidos.Edit;
                             ...
                             // Adiciona todos os números de DI nas informações complementares.
                             with tTmp2 do begin
                                  sql.clear;
                                  sql.add('select pin.DI');
                                  sql.add('      ,Processo = (select Processo from Cybersoft_Cadastros.dbo.ControleNavios where Ordem = pin.Navio)');
                                  sql.add('from PedidosItensNavios pin');
                                  sql.add('where pin.Pedido = :pPed');
                                  sql.add('union all');
                                  sql.add('select pin.DI');
                                  sql.add('      ,Processo = (select Processo from Cybersoft_Cadastros.dbo.ControleNavios where Ordem = pin.Navio)');
                                  sql.add('from PedidosItensNaviosRemessa pin');
                                  sql.add('where pin.Pedido = :pPed');
                                  parambyname('pPed').value := Pedidos.fieldbyName('Numero').asinteger;
                                  //sql.savetofile('c:\temp\Pedidos_Representante_NF_DI.sql');
                                  open;
                                  first;
                                  RelDI := '';
                                  while not eof do begin
                                        RelDI := concat(RelDI, fieldbyname('Processo').asstring,': ', formatMaskText('##/#######-#;0', fieldbyname('DI').asstring), ' | ');
                                        next;
                                  end;
                                  RelDI := copy(RelDI, 1, length(ReldI)-2);
                                  PedidosInf_Complementares2.value := concat(PedidosInf_Complementares2.Value, 'PEDIDO: ', PedidosPedido_Representante.asstring, CHR(13), RelDI);
                             end;
                     Pedidos.Post;
...
procedure TPedidosRepresentantes_PedidoNF.bRemoverClick(Sender: TObject);
begin
      with tQuebras do begin
           if recordcount > 0 then begin
              cAdicionados.value := cAdicionados.value - fieldbyname('Quantidade').AsFloat;
              cSaldo.value       := cSaldo.value + fieldbyname('Quantidade').AsFloat;
              delete;
           end;
      end;
end;

procedure TPedidosRepresentantes_PedidoNF.bSairClick(Sender: TObject);
begin
     close;
end;

procedure TPedidosRepresentantes_PedidoNF.Button1Click(Sender: TObject);
begin
...
procedure TPedidosRepresentantes_PedidoNF.bRemTudoClick(Sender: TObject);
begin
      with tQuebras do begin
           EmptyDataSet;
           cAdicionados.Value := 0;
           cSaldo.Value       := cDisponivel.value;
      end;
end;

procedure TPedidosRepresentantes_PedidoNF.bAdicionarClick(Sender: TObject);
begin
     if cQtde.value > 0 then begin
        if cQtde.value > cSaldo.value then begin
...
procedure TPedidosRepresentantes_PedidoNF.FormClose(Sender: TObject; var Action: TCloseAction);
begin
     PedidosRepresentantes_PedidoNF.release;
     PedidosRepresentantes_PedidoNF := nil
end;

procedure TPedidosRepresentantes_PedidoNF.FormShow(Sender: TObject);
begin
      gFracao.Enabled := mQtdePed = 1;
      ...
      with Dados do begin
           ...
           with tDisponivel do begin
                ...
                sql.add('               ,Navio_Nome = (select Navio from Cybersoft_Cadastros.dbo.ControleNavios cn where cn.Ordem = nv.Navio)');
                sql.add('               ,DI');
                sql.add('               ,Disponivel = round(isnull((select sum(iif(nin.Saida_Entrada = 0, Quantidade, Quantidade*-1)) from NotasItensNavios nin where nin.Navio = nv.Navio and nin.DI = nv.DI), 0)+');
                sql.add('                             isnull((select sum(iif(pin.Saida_Entrada = 0, Quantidade, Quantidade*-1)) from PedidosItensNavios pin where pin.Navio = nv.Navio and pin.DI = nv.DI), 0), 3)');
                sql.add('               ,Data = (select cast(isnull(min(Data), getdate()) as date) from NotasItens ni where ni.Navio = nv.Navio and ni.Codigo_Mercadoria = nv.Codigo_Mercadoria and ni.Saida_Entrada = 1 and isnull(Cancelada, 0) = 0)');
                sql.add('               ,Nota = (select isnull(min(Nota), 0) from NotasItens ni where (ni.Navio = nv.Navio or ni.DI = nv.DI) and ni.Codigo_Mercadoria = nv.Codigo_Mercadoria and ni.Saida_Entrada = 1 and isnull(Cancelada, 0) = 0)');
                sql.add('into #tmp');
                sql.add('from NotasItensNavios nv');
                ...
                sql.add('and (select count(*) from NotasItens ni where ni.Navio = nv.Navio and ni.DI = nv.DI and (select isnull(Remessa, 0) from NotasFiscais nf where nf.Numero = ni.Nota and nf.Data_Emissao = ni.Data) = 1');
                sql.add('                                        and isnull(Cancelada, 0) = 0 and isnull(NFe_Denegada, 0) = 0) > 0');
                sql.add('select * from #tmp order by Data, Nota, DI');
                sql.add('drop table #tmp');
                parambyname('pArmazem').AsInteger := tTemp.FieldByName('Armazem').AsInteger;
                //sql.SaveToFile('c:\temp\Geracao_Representantes_Pedido_Nota_Disponivel.sql');
                open;
                first;
                while not eof do begin
                      cTotal.Value := cTotal.Value + fieldbyname('Disponivel').AsFloat;
                      next;
                end;
           end;
      end;
      ...
      with tTemp do begin
           sql.clear;
           sql.Add('select Qtde = (select sum(Quantidade_Original) from PedidosRepresentantesItens pri where pri.Pedido = :pPed) -');
           sql.Add('              (select isnull(sum(Quantidade), 0) from PedidosRepresentantesItens pri where Pedido_Pai2 = :pPed)');
           parambyname('pPed').value := Pedido_RepresentantesSimp.tSeparados.FieldByName('Pedido').AsString;
           //sql.savetofile('c:\temp\Pedidos_Representantes_PedidoNF.sql');
           open;
           cDisponivel.value := fieldbyName('Qtde').asfloat;
           cSaldo.value      := cDisponivel.value;
      end;
      }
      ...
      with tTemp do begin
           sql.clear;
           sql.Add('select Qtde = (select sum(Quantidade_Original) from PedidosRepresentantesItens pri where pri.Pedido = :pPed) -');
           sql.Add('              (select isnull(sum(Quantidade), 0) from PedidosRepresentantesItens pri where Pedido_Pai = :pPed or Pedido_Pai2 = :pPed)');
           parambyname('pPed').value := Pedido_RepresentantesSimp.tSeparados.FieldByName('Pedido').AsString;
           //sql.savetofile('c:\temp\Pedidos_Representantes_PedidoNF.sql');
           open;
           cDisponivel.value := fieldbyName('Qtde').asfloat;
...
Function TPedidosRepresentantes_PedidoNF.MontaComplemento(Texto: String; pCampo:integer):String;
Var
    mTexto,
    mBusca,
    mTroca,
...
Function TPedidosRepresentantes_PedidoNF.CalculaMacro(Campo:String):Real;
var
    mPos   
   ,mTam   
   ,mPosFun
...
procedure TPedidosRepresentantes_PedidoNF.CalculaImpostos;
Var
    mUF,
    mCST: String;
    mApuracao1,
...
begin
      With Dados, dmFiscal do begin
           ...
           if PedidosItensValor_ICMSMono.ascurrency = 0 then begin
              PedidosItensPercentual_ICMSMono.value := 0;
              PedidosItensValor_BCICMSMono.value    := 0;
           end;
           }
           if PedidosItensValor_ICMSMonoRet.ascurrency = 0 then begin                 
...
procedure TPedidosRepresentantes_PedidoNF.CalculoBCR;
Var
   mUF : String;
   mBCR: Real;
begin
...
procedure TPedidosRepresentantes_PedidoNF.TotalizaPedido;
Var
    mVolume_PesoLiquido,
    mVolume_PesoBruto,
    mRateio_Descontos,
...
// Retorno o navio com a menora quantidade disponivel no estoque.
procedure TPedidosRepresentantes_PedidoNF.SalvaNavio(Quantidade: real; Navio: integer; DI, BL: string);
begin 
     with Dados do begin
          if PedidosItens.FieldByName('Movimenta_Estoque').asboolean then begin
             // Para pedidos que os itens movimentam estoque. 
             with PedidosItensNavios do begin
                  with tTemp do begin
                       sql.Clear;
                       sql.Add('select isnull(max(Registro), 0)+1 as Reg from PedidosItensNavios');
                       Open;
                  end;
                  append;
                        ...
                        fieldbyname('Quantidade').value        := Quantidade;
                        fieldbyname('Navio').value             := Navio;
                        fieldbyname('DI').value                := DI;
                        fieldbyname('BL').value                := BL;
                  post;
                  with PedidosItens do begin
                       edit;
                          fieldbyname('Navio').value := Navio;
             ...
             // Para pedidos que os itens não movimentam estoque. 
             with PedidosItensNaviosRemessa do begin
                  with tTemp do begin
                       sql.Clear;
                       sql.Add('select isnull(max(Registro), 0)+1 as Reg from PedidosItensNaviosRemessa');
                       Open;
                  end;
                  append;
                        fieldbyname('Registro').value          := tTemp.FieldByName('Reg').AsInteger;
...
procedure TPedidosRepresentantes_PedidoNF.StaticText1Click(Sender: TObject);
begin
...
procedure TPedidosRepresentantes_PedidoNF.CopiaPedidos;
var
   i:integer;
...
begin
     ...
     while not tQuebras.Eof do begin 
           ...
           with tTemp do begin
                ...
                with tTmp2 do begin
                     sql.Clear;
                     sql.Add('update PedidosRepresentantesItens set Quantidade = Quantidade - :pQtde');
                     sql.add('where Pedido = :pPed');
                     parambyname('pPed').Value    := mPedPai;
                     parambyname('pQtde').asfloat := tQuebras.fieldbyname('Quantidade').asfloat;
                     execute;
                end;
                }
           end;
     ...
     with tTmp2 do begin
          ...
          sql.Clear;
//          sql.Add('update PedidosRepresentantesItens set Quantidade = Quantidade_Original - (select sum(Quantidade) from PedidosRepresentantesItens pri where pri.Pedido_Pai2 = :pPedPai and pri.Codigo_Mercadoria = Codigo_Mercadoria)');
          ...
          sql.Clear;
//          sql.Add('update PedidosRepresentantes set Total_Pedido = Total_PedidoOriginal - isnull((select sum(Valor_Unitario * Quantidade) from PedidosRepresentantesItens pri where pri.Pedido_Pai2 = :pPedPai), 0)');
...
procedure TPedidosRepresentantes_PedidoNF.cTipoNotaExit(Sender: TObject);
begin
      with Dados do begin
           if Trim(TipoNota.FieldByName('CFOP_Dentro').asstring) <> '' then begin
               cCFOPDentro.KeyValue := TipoNota.FieldByName('CFOP_Dentro').AsString;
...
procedure TPedidosRepresentantes_PedidoNF.GeraDuplicatas(Pedido: integer; Valor: real);
Var
    mValorParcela,
    mValorProduto,
    mValorIPI,
