...
type
  ...
  public
    function Add: TR4099CollectionItem; overload; deprecated
    {$IFDEF SUPPORTS_DEPRECATED_DETAILS} 'Obsoleta: Use a função New'{$ENDIF};
    function New: TR4099CollectionItem;

    property Items[Index: Integer]: TR4099CollectionItem read GetItem
  ...
  { TevtFech }
  TevtFech = class(TReinfEvento)
  // Classe do elemento principal do XML do evento!
  private
...
begin
  ...
  Gerador.wCampo(tcStr, '', 'fechRet', 1, 1, 1,
    tpFechRetToStr(FinfoFech.fechRet));
  Gerador.wGrupo('/infoFech');
end;
...
begin
  ...
  try
    ...
    begin
      ...
      ideEvento.ProcEmi := StrToProcEmiReinf(Ok, INIRec.ReadString(sSecao,
        'procEmi', '1'));
      ideEvento.VerProc := INIRec.ReadString(sSecao, 'verProc', EmptyStr);

      sSecao := 'ideContri';
      ideContri.TpInsc := StrToTpInscricao(Ok, INIRec.ReadString(sSecao,
        'tpInsc', '1'));
      ...
      sSecao := 'infoFech';
      infoFech.fechRet := StrTotpFechRet(Ok, INIRec.ReadString(sSecao,
        'fechRet', '0'));
    end;
