...
uses
  ...
  Mask, Grids, DBGrids, MemDS, RxToolEdit, Vcl.JumpList, RxCurrEdit, Math, system.UITypes, System.WideStrUtils, System.JSON, Xml.XMLDoc, Xml.XMLIntf;

type
  TProcesso_RegistroDI = class(TForm)
    Panel1: TPanel;
    bSair: TButton;
    Navega: TDBNavigator;
    bPesquisa: TButton;
    ...
    procedure bGerarDUIMPClick(Sender: TObject);
  private
    ...
    ,mIPI
    ,mII: real;
    JsonDUIMP: TJSONObject;
    procedure CalculaImpostos;
    procedure CriaObjetoJson(pPai: TJSONObject; const pNome: string; const pCampos, pValores: array of string);
  public
    { Public declarations }
  end;
  
var
  Processo_RegistroDI: TProcesso_RegistroDI;

implementation

Uses Funcoes, frmDados, frmDMDespacho, frmPesquisaGerais, frmMenu_Principal, frmDMFiscal;

{$R *.dfm}

procedure TProcesso_RegistroDI.bSairClick(Sender: TObject);
begin
      close;
end;

procedure TProcesso_RegistroDI.DBEdit6Change(Sender: TObject);
begin
     with Dados, dmDespacho do begin
          ...
          with tTotalAcre do begin
               ...
               Parambyname('pDI').Asstring := DIDI.asstring;
               Parambyname('pData').Asdate := DIData.asdatetime;
...
procedure TProcesso_RegistroDI.NavegaAcreBeforeAction(Sender: TObject; Button: TNavigateBtn);
begin
      ActiveControl := nil;
      with Dados, dmDespacho do begin
           if Button = nbPost then begin
              if DIAcrescimo.state = dsInsert then begin
                 ...
                 DIAcrescimoDI.Value          := DIDI.Asstring;
               end;
...
procedure TProcesso_RegistroDI.NavegaAcreClick(Sender: TObject; Button: TNavigateBtn);
var
   i:integer;
begin
...
procedure TProcesso_RegistroDI.FormClose(Sender: TObject;var Action: TCloseAction);
begin
...
procedure TProcesso_RegistroDI.FormCreate(Sender: TObject);
begin
      If FileExists('fundo_barra.bmp') then Image1.Picture.LoadFromFile('fundo_barra.bmp');
end;

procedure TProcesso_RegistroDI.FormShow(Sender: TObject);
begin
     with Dados, dmDespacho do begin
          ...
          with DocumentoTransp do begin
               SQL.Clear;
               SQL.Add('select * from DocumentoTransporte order by Descricao');
               Open;
          end;
          with MotivoSit do begin
               sql.Clear;
               sql.Add('select * from MotivoSituacao order by Descricao');
               open;
          end;
...
procedure TProcesso_RegistroDI.NavegaBeforeAction(Sender: TObject;Button: TNavigateBtn);
begin
      with Dados, dmDespacho do begin
           if (Button = nbPost) and (DI.state = dsInsert) then begin
              tReg.SQL.Clear;
              tReg.SQL.Add('SELECT Reg = ISNULL(MAX(Registro), 0)+1 FROM DI');
              tReg.Open;
              DIRegistro.Value := tReg.FieldByName('Reg').asinteger;
              if trim(DIDI.AsString) = '' then begin
                 DIDI.Value := concat('TMP#', trim(tReg.FieldByName('Reg').asstring));
              end;
           end;
           if Button = nbDelete then begin
              ...
              tReg.ParamByName('pReg').AsInteger := DIRegistro.AsInteger;
              tReg.Execute;
              DIAcrescimo.Refresh;
              tReg.SQL.Clear;
              tReg.SQL.Add('delete from DIAdicoes where DI = :pDI');
              tReg.ParamByName('pDI').Asstring := DIDI.asstring;
              tReg.Execute;
              DIAdicoes.Refresh;
           end;
           Panel2.Enabled := true;
...
procedure TProcesso_RegistroDI.NavegaClick(Sender: TObject;Button: TNavigateBtn);
var
   i:integer;
begin
      with Dados, dmDespacho do begin
           if (Button = nbEdit) or (Button = nbInsert) then begin
              ...
              if Button = nbInsert then begin
                 DIFUNDAP.Value           := false;
                 DIMultimodal.Value       := false;
                 DISituacao.value         := 'Em Confecção';
                 DIOrigem_DI.VAlue        := 2;
                 DITipo_Pagamento.Value   := 1;
              end;
...
procedure TProcesso_RegistroDI.NavegaDOCBeforeAction(Sender: TObject; Button: TNavigateBtn);
begin
      ActiveControl := nil;
      with Dados, dmDespacho do begin
           if Button = nbPost then begin
              if DIDocumentos.state = dsInsert then begin
                 DIDocumentosDI.Value := DIDI.Asstring;
               end;
           end;
           if Button = nbDelete then begin
              if MessageDlg('Deseja realmente excluir este registro?', mtConfirmation, [mbYes, mbNo], 0) = mrNo then begin
...
procedure TProcesso_RegistroDI.NavegaDOCClick(Sender: TObject; Button: TNavigateBtn);
var
   i:integer;
begin
      with dmDespacho do begin
...
procedure TProcesso_RegistroDI.RxDBLookupCombo7Change(Sender: TObject);
begin
     with dmDespacho do begin
          with tSetor do begin
               SQL.Clear;
               ...
               sql.Add('order by Descricao');
               ParamByName('pRecinto').AsInteger := DIRecinto_Aduaneiro.AsInteger;
               Open;
          end;
     end;
end;

procedure TProcesso_RegistroDI.bPesqRecClick(Sender: TObject);
begin
      ...
      PesquisaGerais.mTabela := dmDespacho.DI;
      PesquisaGerais.Showmodal;
end;

procedure TProcesso_RegistroDI.bPesqURFDespClick(Sender: TObject);
begin
      PesquisaGerais         := TPesquisaGerais.Create(Self);
...
procedure TProcesso_RegistroDI.bPesqURFEntClick(Sender: TObject);
begin
      PesquisaGerais         := TPesquisaGerais.Create(Self);
      PesquisaGerais.mTab    := 'LocalDesembaraco';
      PesquisaGerais.mCampo2 := 'Descricao';
...
procedure TProcesso_RegistroDI.bPesqConsClick(Sender: TObject);
begin
      PesquisaGerais         := TPesquisaGerais.Create(Self);
...
procedure TProcesso_RegistroDI.bPesqPaisClick(Sender: TObject);
begin
      PesquisaGerais         := TPesquisaGerais.Create(Self);
...
procedure TProcesso_RegistroDI.bGerarAdicoesClick(Sender: TObject);
var
   mPesoLiq
  ,mPesoBrt
  ,mTotalAcre
...
begin
     ...
     with dmDespacho, Dados, dmFiscal do begin
          with tAdicoes do begin
               ...
               parambyname('pProc').AsString := DIProcesso.AsString;
          ...
          with ttmp do begin
               ...
               parambyname('pDI').AsString := iif(trim(DIDI.AsString) <> '', DIDI.AsString, 'TMP#'+DIRegistro.AsString);
               //sql.SaveToFile('c:\temp\ttmp_delete.sql');
               execute;
          end;
          with DIAdicoes do begin
               sql.Clear;
                     ...
                     if tAcrescimos.RecordCount > 0 then begin
                        with AdicoesAcre do begin
                             ...
                             Append;
                                  ...
                                  FieldByname('DI').Value         := iif(trim(DIDI.AsString) <> '', DIDI.AsString, 'TMP#'+DIRegistro.AsString);
                                  ...
                                  FieldByname('Valor_ME').Value   := (tAcrescimos.fieldbyname('Total_ME').AsCurrency   / DIPeso_Liquido.AsFloat) * tAdicoes.FieldByName('Peso_Liquido').AsFloat;
                                  FieldByname('Valor_Real').Value := (tAcrescimos.fieldbyname('Total_Real').AsCurrency / DIPeso_Liquido.AsFloat) * tAdicoes.FieldByName('Peso_Liquido').AsFloat;
                             post;
                             tReg.Close;
                        end;
                     end;
...
procedure TProcesso_RegistroDI.bGerarDIClick(Sender: TObject);
var
  mAcreReal: real;
  mDesc: string;
  mDI: TStringList;
           ...
           with Memo.lines do begin
                ...
                with tAdicoes do begin
                     ...
                     while not eof do begin
                           ...
                           add(CriaTag('dadosCargaPaisProcedenciaCodigo'             , Copy(DIPais_Procedencia.asstring, 1, 3), 'D', 3));
                           ...
                           add(CriaTag('iiFundamentoLegalCodigo'                     , formatfloat('00', DI.FieldByName('Fundamento_Legal').asinteger), 'D', 3));
                           add(CriaTag('iiMotivoAdmissoTemporariaCodigo'             , formatfloat('00', DI.FieldByName('Motivo_AdmissaoTemp').asinteger), 'D', 3));
                ...
                with RecintosAduaneiros do begin
                     sql.clear;
                     sql.Add('select * from RecintosAduaneiros where Codigo = '+iif(DI.Fieldbyname('Recinto_Aduaneiro').asstring <> '', DI.Fieldbyname('Recinto_Aduaneiro').asstring, quotedstr('')));
                     open;
                end;
                with DI do begin
...
//                                      'FOB..............:', FieldByName('Moeda_FOB').asstring, '   ', cMoedaFOB.       , #13,
                ...
                SalvaComoUTF8('c:\temp\Cybersoft_DI_'+DIDI.value+'.xml', Memo.Lines);
           end;
      end;
end;

procedure TProcesso_RegistroDI.bGerarDUIMPClick(Sender: TObject);
var
   Identificacao
  ,Importador
  ,Carga   
  ...
  ,CambiaisMotivo
  ,CambiaisCobertura
  ,AtribDuimpItem: TJSONObject;
  
   Itens
  ,DocumentosInstrucaoArr
  ,PalavrasChaveArr
  ,AcrescDeduc
  ,LPCOS
  ,CertMercoSul
  ,DeclaraVinc
  ,AtribDuimp: TJSONArray;  
...
begin
     pPasta.ActivePageIndex := 7;
     with Dados, dmDespacho, DI do begin
          ...
          with tMercadoria do begin
               sql.Clear;
               sql.Add('select ivi.Item');
               sql.Add('      ,ivi.Codigo_Mercadoria');
               sql.Add('      ,ivi.Quantidade');
               sql.add('      ,ivi.Valor_UnitarioME');
               sql.add('      ,Unidade = (select max(Descricao) from Cybersoft_Cadastros.dbo.UnidadeMedida where Codigo = ivi.Unidade)');
               sql.add('      ,Descricao = replace(prd.Descricao, ''<{''+cast(prd.Codigo as varchar(20))+''}>'', '''')');
               sql.add('      ,ivi.Fabricante');
               sql.add('      ,inv.Exportador');
               sql.add('      ,relacao_FabExp = iif(fab.Codigo_Estrangeiro = frn.Codigo_Estrangeiro, ''EXPORTADOR_IGUAL_FABRICANTE'', ''EXPORTADOR_DIFERENTE_FABRICANTE'')');
               sql.add('      ,Fab_CodEstrangeiro = (select iif(fab.Pais = ''1058'', fab.CNPJ, fab.Codigo_Estrangeiro))');
               sql.add('      ,Fab_NumIdentif     = fab.Ident_Operador');
               sql.Add('      ,Exp_CodEstrangeiro = (select iif(frn.Pais = ''1058'', frn.CNPJ, frn.Codigo_Estrangeiro))');
               sql.Add('      ,Exp_NumIdentif     = frn.Ident_Operador');
               sql.Add('      ,ivi.Vinculo_CompVend');
               sql.Add('      ,inv.Aplicacao');
               sql.Add('      ,inv.Condicao_Mercadoria');
               sql.Add('      ,Qtde_Estat = (select isnull(Quantidade_Estatistica, 1) from NCM nc where nc.NCM = ivi.NCM)');
               sql.Add('      ,ivi.Peso_Liquido');
               sql.Add('      ,Moeda_Sigla = (select Simbolo from Cybersoft_Cadastros.dbo.Moedas where Codigo = inv.Moeda)');
               sql.Add('      ,ivi.Valor_UnitarioME');
               sql.add('      ,inv.Metodo_Valoracao');
               sql.add('      ,inv.INCOTERMS');
               sql.Add('      ,Frete_Item  = round((((isnull(inv.Frete_Collect, 0) + isnull(inv.Frete_PrePaid, 0)) / inv.Peso_Liquido) * ivi.Peso_Liquido) * ivi.quantidade, 2, 4)');
               sql.Add('      ,Seguro_Item = round(((inv.Seguro / inv.Total_FaturaME) * ivi.Valor_UnitarioME) * ivi.Quantidade, 2, 4)');
               sql.Add('      ,DIAcrescimo =  round((((select sum(isnull(Valor_ME, 0)) from DIAcrescimo dia where dia.DI = :pDI) / cast(inv.Total_FaturaME as float) * ivi.Valor_UnitarioME)) * ivi.Quantidade, 2)');
               sql.add('      ,ivi.Certificado_MercoSulTipo');
               sql.add('      ,ivi.Certificado_MercoSulNumero');
               sql.add('      ,ivi.Certificado_MercoSulQtde');
               sql.add('      ,inv.Cobertura_Cambial');
               sql.add('      ,inv.Numero_ROF');
               sql.add('      ,inv.Valor_CoberturaCambial');
               sql.add('      ,inv.Financiadora');
               sql.add('      ,inv.Motivo_SemCobertura');
               sql.add('      ,ivi.NCM');
               sql.Add('from InvoiceItens ivi, Invoice inv, Fabricantes fab, Fornecedores frn, Produtos prd');
               sql.Add('where inv.Processo = :pProc');
               sql.Add('and ivi.Codigo_Mercadoria = prd.Codigo');
               sql.Add('and inv.Numero = ivi.Invoice');
               sql.Add('and fab.Codigo = ivi.Fabricante');
               sql.add('and frn.Codigo = inv.Exportador');
               sql.add('order by Item');
               parambyname('pProc').AsString := DIProcesso.AsString;
               parambyname('pDI').AsString   := DIDI.AsString;
               //sql.SaveToFile('c:\temp\DUIMP_tMercadoria.sql');
               open;
          end;

          case DI.FieldByName('Modalidade_Importacao').AsInteger of
               1: mModalImp := 'IMPORTACAO_DIRETA';
               2: mModalImp := 'IMPORTACAO_POR_CONTA_E_ORDEM';
               3: mModalImp := 'IMPORTACAO_POR_ENCOMENDA';
          end;          
     
          //====[ DADOS GERAIS DO DUIMP ]===================================================================================================================\\
          
          JsonDUIMP := TJSONObject.Create;
          
          Paises.Locate('Codigo', fieldbyname('Pais_Procedencia').AsInteger, [loCaseInsensitive]);
          
          Moedas.Locate('Codigo', fieldbyname('Moeda_FOB').AsInteger, [loCaseInsensitive]);
          mSimbMoedaFOB := Moedas.fieldbyname('Simbolo').AsString;
          Moedas.Locate('Codigo', fieldbyname('Moeda_Frete').AsInteger, [loCaseInsensitive]);
          mSimbMoedaFrete := Moedas.fieldbyname('Simbolo').AsString;
          Moedas.Locate('Codigo', fieldbyname('Moeda_Seguro').AsInteger, [loCaseInsensitive]);
          mSimbMoedaSeg := Moedas.fieldbyname('Simbolo').AsString;
          
          mCompl := concat('OPERACAO POR CONTA PROPRIA ENCOMENDANTE/ADQUIRENTE: ECB TRADE COMÉRCIO INTL LTDA.', #13,
                           'CNPJ: ', Empresas.fieldbyname('cnpj').asstring, 'FECHAMENTO DE CAMBIO SOB RESPONSABILIDADE DO ENCOMENDANTE/ADQUIRENTE.', #13,
                           StringOfChar('=', 50), #13,
                           'A MERCADORIA CONSTANTE NESTE NAVIO ESTÁ ACONDICIONADA EM:',
                           'TAXA DE CAMBIO', #13,
                           StringOfChar('=', 50), #13,
                           'DATA: ', fieldbyname('Data').AsString, #13,
                           stringofChar('=', 50), ' MOEDAS UTILIZADAS ', stringofchar('=', 50), #13,
                           'FOB..............:', fieldbyname('Moeda_FOB').asstring, '   ', mSimbMoedaFOB, #13);
          ...
          Identificacao.AddPair('informacaoComplementar', mCompl);
          JsonDUIMP.AddPair('identificacao', Identificacao);
          
          UnidadeDeclarada := TJSONObject.Create;
          UnidadeDeclarada.AddPair('codigo', fieldbyname('Recinto_Aduaneiro').AsString);

          Carga := TJSONObject.Create;
          Carga.AddPair('tipoIdentificacaoCarga', fieldbyname('Tipo_DocCarga').AsString);
          Carga.AddPair('identificacao'         , fieldbyname('Numero_DocCarga').AsString);
          Carga.AddPair('unidadeDeclarada'      , UnidadeDeclarada);
          JsonDUIMP.AddPair('Carga', Carga);
          
          MotivoEspecial := TJSONObject.Create;
          Carga.AddPair('motivoSituacaoEspecial', fieldbyname('Motivo_SitEspecial').AsString);

          Seguro := TJSONObject.Create;
          Seguro.AddPair('codigoMoedaNegociada', mSimbMoedaSeg);
          Seguro.AddPair('valorMoedaNegociada' , stringreplace(formatfloat('##0.00', fieldbyName('Seguro_ME').ascurrency), ',', '.', [rfReplaceAll]));
          JsonDUIMP.AddPair('seguro', Seguro);

          Frete := TJSONObject.Create;
          Frete.AddPair('codigoMoedaNegociada', mSimbMoedaFrete);
          Frete.AddPair('valorMoedaNegociada' , stringreplace(formatfloat('##0.00', fieldbyName('Frete_Collect').ascurrency), ',', '.', [rfReplaceAll]));
          JsonDUIMP.AddPair('frete', Frete);

          PaisProc := TJSONObject.Create;
          PaisProc.AddPair('codigo', Paises.FieldByName('Sigla').AsString);
          JsonDUIMP.AddPair('paisProcedencia', PaisProc);

          Documentos := TJSONObject.Create;
          JsonDUIMP.AddPair('documentos', Documentos);
          DocumentosInstrucaoArr := TJSONArray.Create;
          Documentos.AddPair('documentosInstrucao', DocumentosInstrucaoArr);
          DocumentoInstrucao := TJSONObject.Create;
          DocumentosInstrucaoArr.AddElement(DocumentoInstrucao);
          
          with DIDocumentos do begin
               while not eof do begin
                     Tipo := TJSONObject.Create;
                     Tipo.AddPair('codigo', fieldbyname('Codigo').asstring);
                     DocumentoInstrucao.AddPair('tipo', Tipo);
                     next;
               end;
          end;
          PalavrasChaveArr := TJSONArray.Create;
          DocumentoInstrucao.AddPair('palavrasChave', PalavrasChaveArr);

          PalavraChave := TJSONObject.Create;
          PalavraChave.AddPair('codigo', TJSONNumber.Create(33));
          PalavraChave.AddPair('valor' , '9999.99.99');
          PalavrasChaveArr.AddElement(PalavraChave);

          cMemDUIMP.Lines.Text := JsonDUIMP.format(2);
          cMemDUIMP.Lines.SaveToFile('c:\temp\DUIMP_Capa.txt');
          cMemDUIMP.Lines.Clear;

          //====[ PRODUTOS ]===================================================================================================================\\
          JsonDUIMP := TJSONObject.Create;
          with tMercadoria do begin
               while not eof do begin
                     ItemIdent := TJSONObject.Create;
                     ItemIdent.AddPair('numeroItem'   ,TJSONNumber.Create(FieldByName('Item').AsInteger));
                     JsonDUIMP.AddPair('identificacao', ItemIdent); 

                     Produto := TJSONObject.Create;
                     Produto.AddPair('codigo'       , TJSONNumber.Create(fieldbyname('Codigo_Mercadoria').AsInteger)); 
                     Produto.AddPair('versao'       , '1'); 
                     Produto.AddPair('niResponsavel', copy(Empresas.FieldByName('CNPJ').AsString, 1, 8) ); 
                     JsonDUIMP.AddPair('produto', Produto); 

                     Clientes.Locate('Codigo', DI.FieldByName('Adquirente').AsInteger, [loCaseInsensitive]);
          
                     caractImp := TJSONObject.Create;
                     caractImp.AddPair('indicador', mModalImp);
          
                     if DI.fieldbyname('Modalidade_Importacao').AsInteger <> 1 then begin
                        caractImp.AddPair('ni', Clientes.fieldbyname('CNPJ').AsString);
                     end;
                     JsonDUIMP.AddPair('caracterizacaoImportacao', caractImp); 

                     indExpFab := TJSONObject.Create;
                     indExpFab.AddPair('codigo'                       , fieldbyname('relacao_FabExp').asstring);
                     JsonDUIMP.AddPair('indicadorExportadorFabricante', indExpFab);

                     Fabricante := TJSONObject.Create;
                     Fabricante.AddPair('codigo'    , fieldbyname('Fab_CodEstrangeiro').AsString);
                     Fabricante.AddPair('versao'    , '1');
                     Fabricante.AddPair('nioperador', fieldbyname('Fab_NumIdentif').AsString);
                     Fabricante.AddPair('pais'      , Paises.FieldByName('Sigla').asstring);
                     JsonDUIMP.AddPair('fabricante', Fabricante);
                     
                     Exportador := TJSONObject.Create;
                     Exportador.AddPair('codigo'    , fieldbyname('Fab_CodEstrangeiro').AsString);
                     Exportador.AddPair('versao'    , '1');
                     Exportador.AddPair('nioperador', fieldbyname('Fab_NumIdentif').AsString);
                     Exportador.AddPair('pais'      , Paises.FieldByName('Sigla').asstring);
                     JsonDUIMP.AddPair('exportador', Exportador);

                     IndCompVend := TJSONObject.Create;
                     IndCompVend.AddPair('codigo'                  , fieldbyname('Vinculo_CompVend').AsString);
                     JsonDUIMP.AddPair('indicadorCompradorVendedor', IndCompVend);

                     
                     // Remove quebra de linhas carriage return do texto da descrição.
                     mDescricao := stringreplace(fieldbyname('Descricao').asstring, char(10), '', [rfReplaceAll]);
                     mDescricao := stringreplace(mDescricao, char(11), '', [rfReplaceAll]);
                     mDescricao := trim(stringreplace(mDescricao, char(12), '', [rfReplaceAll]));
                     
                     Mercadoria    := TJSONObject.Create;
                     MercTipoAplic := TJSONObject.Create;
                     MercMoeda     := TJSONObject.Create;
                     MercTipoAplic.AddPair('codigo'                  , fieldbyname('Aplicacao').asstring);
                     MercMoeda.AddPair('codigo'                      , 'USD');
                     Mercadoria.AddPair('tipoAplicacao'              , MercTipoAplic);
                     Mercadoria.AddPair('condicao'                   , fieldbyname('Condicao_Mercadoria').asstring);
                     Mercadoria.AddPair('unidadeComercial'           , removeacentos(fieldbyname('Unidade').AsString));
                     Mercadoria.AddPair('quantidadeComercial'        , TJSONNumber.Create(fieldbyname('Quantidade').asfloat));
                     Mercadoria.AddPair('quantidadeMedidaEstatistica', TJSONNumber.Create(SimpleRoundTo(fieldbyname('Quantidade').asfloat * fieldbyname('Qtde_Estat').asfloat, -6)));
                     Mercadoria.AddPair('pesoLiquido'                , TJSONNumber.Create(fieldbyname('Peso_Liquido').asfloat * fieldbyname('Quantidade').asfloat));
                     Mercadoria.AddPair('moedaNegociada'             , MercMoeda);
                     Mercadoria.AddPair('valorUnitarioMoedaNegociada', TJSONNumber.Create(fieldbyname('Valor_UnitarioME').asfloat));
                     Mercadoria.AddPair('descricao'                  , mDescricao);
                     JsonDUIMP.AddPair('mercadoria', Mercadoria);

                     CondicaoVenda   := TJSONObject.Create;
                     MetodoValoracao := TJSONObject.Create;
                     CondIncoterm    := TJSONObject.Create;
                     CondFrete       := TJSONObject.Create;
                     CondSeguro      := TJSONObject.Create;
                     AcrescDeduc     := TJSONArray.Create;
                     AcrescimosVal   := TJSONObject.Create;
                     
                     MetodoValoracao.AddPair('codigo'  , TJSONNumber.Create(fieldbyname('Metodo_Valoracao').asinteger));
                     CondicaoVenda.AddPair('metodoValoracao', MetodoValoracao);

                     CondIncoterm.AddPair('codigo'     , fieldbyname('INCOTERMS').asstring);
                     CondIncoterm.AddPair('complemento', '');
                     CondicaoVenda.AddPair('incoterm'  , CondIncoterm);

                     CondFrete.AddPair('valor'   , TJSONNumber.Create(fieldbyname('Frete_Item').asfloat));
                     CondicaoVenda.AddPair('frete', CondFrete);

                     CondSeguro.AddPair('valor'   , TJSONNumber.Create(fieldbyname('Seguro_Item').asfloat));
                     CondicaoVenda.AddPair('seguro', CondSeguro);

                     // Acrescimos e Deduções.
                     with ttmp do begin
                          sql.clear; 
                          sql.Add('select dia.Codigo');
                          sql.Add('      ,Moeda_Sigla = (select Simbolo from Cybersoft_Cadastros.dbo.Moedas where Codigo = inv.Moeda)');
                          sql.Add('      ,dia.DI');
                          sql.Add('      ,dia.Valor_ME');
                          sql.Add('      ,Rateio = round(((dia.Valor_ME / cast(inv.Total_FaturaME as float)) * ivi.Valor_UnitarioME) * ivi.Quantidade, 2)');
                          sql.Add('from DIAcrescimo dia, DI, Invoice inv, InvoiceItens ivi');
                          sql.Add('where dia.DI = :pDI');
                          sql.Add('and di.di = dia.DI');
                          sql.Add('and inv.Numero = DI.Fatura_Numero');
                          sql.Add('and ivi.Invoice = DI.Fatura_Numero');
                          sql.Add('and ivi.Codigo_Mercadoria = :pCod');
                          parambyname('pDI').AsString  := DIDI.AsString;
                          parambyname('pCod').value := tMercadoria.FieldByName('Codigo_Mercadoria').AsInteger;
                          open;
                          first;
                          while not eof do begin
                                // Objeto do item
                                AcrescimoItems := TJSONObject.Create;
                                AcrescimoItems.AddPair('tipo', 'ACRESCIMO');

                                // Objeto moeda
                                AcrescimoMoeda := TJSONObject.Create;
                                AcrescimoMoeda.AddPair('codigo', fieldbyname('Moeda_Sigla').AsString);
                                AcrescimoMoeda.AddPair('valor' , TJSONNumber.Create(fieldbyname('Rateio').AsFloat));
                                AcrescimoItems.AddPair('moeda' , AcrescimoMoeda);

                                // Objeto denominacao
                                AcrescimoDenom := TJSONObject.Create;
                                AcrescimoDenom.AddPair('codigo'     , TJSONNumber.Create(fieldbyname('Codigo').asinteger));
                                AcrescimoItems.AddPair('denominacao', AcrescimoDenom);

                                // Adiciona o item ao array
                                AcrescDeduc.Add(AcrescimoItems);
                                next;
                          end;
                     end;
                     CondicaoVenda.AddPair('acrescimosDeducoes', AcrescDeduc);
                     JsonDUIMP.AddPair('condicaoVenda', CondicaoVenda);
                     
                     // LPCO's.
                     LPCOS := TJSONArray.Create;
                     with ttmp do begin
                          sql.clear; 
                          sql.Add('select Numero');
                          sql.Add('from LPCO');
                          sql.Add('where LPCO in(select LPCO from InvoiceItensLPCO where Codigo_Mercadoria = '+tMercadoria.FieldByName('Codigo_Mercadoria').asstring+')');
                          //sql.SaveToFile('c:\temp\DUIMP_JSON_LPCOS.sql');
                          open;
                          first;
                          while not eof do begin
                                // Objeto do item
                                LPCOSItem := TJSONObject.Create;
                                LPCOSItem.AddPair('numero', fieldbyname('Numero').AsString);
                                LPCOS.AddElement(LPCOSItem);
                                next;
                          end;
                     end;
                     JsonDUIMP.AddPair('lpcos', LPCOS);
                     
                     // Certificado Mercosul.
                     if trim(fieldbyname('Certificado_MercosulNumero').AsString ) <> '' then begin
                        CertMercoSul     := TJSONArray.Create;
                        CertMercoSulItem := TJSONObject.Create;
                        CertMercoSulItem.AddPair('tipo'      , fieldbyname('Certificado_MercosulTipo').AsString);
                        CertMercoSulItem.AddPair('numero'    , fieldbyname('Certificado_MercosulNumero').AsString);
                        CertMercoSulItem.AddPair('quantidade', TJSONNumber.Create(fieldbyname('Certificado_MercosulQtde').AsFloat));
                        CertMercoSul.AddElement(CertMercoSulItem);
                        JsonDUIMP.AddPair('certificadoMercosul', CertMercoSul);
                     end;
                     
                     // Declarações vinculadas.
                     {
                     DeclaraVinc := TJSONArray.Create;
                     DeclaraVincItem := TJSONObject.Create;
                     DeclaraVincItem.AddPair('tipo'       , 'DUIMP');
                     DeclaraVincItem.AddPair('numero'     , '25BR00001010550-TESTE');
                     DeclaraVincItem.AddPair('numeroItem' , TJSONNumber.Create(1));
                     DeclaraVincItem.AddPair('chaveAcesso', '23NcT000115501-TESTE');
                     DeclaraVinc.AddElement(DeclaraVincItem);
                     JsonDUIMP.AddPair('declaracoesVinculadas', DeclaraVinc);
                     }
                     
                     // Dados cambiais.
                     Cambiais          := TJSONObject.Create;
                     CambiaisCobertura := TJSONObject.Create;
                     CambiaisFinanc    := TJSONObject.Create;
                     CambiaisMotivo    := TJSONObject.Create;
                     
                     CambiaisCobertura.AddPair('codigo', fieldbyname('Cobertura_Cambial').AsString);
                     Cambiais.AddPair('coberturaCambial', CambiaisCobertura);
                     Cambiais.AddPair('numeroROF', fieldbyname('Numero_ROF').AsString);
                     CambiaisFinanc.AddPair('codigo', TJSONNumber.Create(fieldbyname('Financiadora').asfloat));
                     Cambiais.AddPair('instituicaoFinanciadora', CambiaisFinanc);
                     Cambiais.AddPair('valorCoberturaCambial', TJSONNumber.Create(fieldbyname('Valor_CoberturaCambial').AsFloat));
                     CambiaisMotivo.AddPair('codigo', TJSONNumber.Create(fieldbyname('Motivo_SemCobertura').asfloat));
                     Cambiais.AddPair('motivoSemCobertura', CambiaisMotivo);
                     JsonDUIMP.AddPair('dadosCambiais', Cambiais);

                     // Atributos da DUIMP.
                     with ttmp do begin
                          sql.clear; 
                          sql.Add('select distinct * ');
                          sql.Add('from cybersoft_cadastros.dbo.NCMAtributos');
                          sql.Add('where NCM = '+tMercadoria.FieldByName('NCM').AsString);
                          //sql.Add('where NCM = ''84439933'' ');
                          //sql.SaveToFile('c:\temp\DUIMP_JSON_LPCOS.sql');
                          open;
                          first;
                          if recordcount > 0 then begin
                             AtribDuimp := TJSONArray.Create;
                             while not eof do begin
                                   // Objeto do item
                                   AtribDuimpItem := TJSONObject.Create;
                                   AtribDuimpItem.AddPair('codigo', fieldbyname('Atributo').AsString);
                                   AtribDuimpItem.AddPair('valor', fieldbyname('NCM').AsString);
                                   AtribDuimp.AddElement(AtribDuimpItem);
                                   next;
                             end;
                             JsonDUIMP.AddPair('atributosDuimp', AtribDuimp);
                          end;
                     end;

                     (* 
                       *********************************************[ ESTUDAR O CASO PARA IMPLMENTAÇÃO ]**************************************************
...
*)
          ...
          cMemDUIMP.Lines.Text := JsonDUIMP.format(2);
          cMemDUIMP.Lines.SaveToFile('c:\temp\DUIMP_Itens.txt');
          //=================================================================================================================================================\\
     end;
...
*      {
*        "tipo": "DUIMP",
*        "numero": "23BR00001010550",
*        "numeroItem": 10001,
*        "chaveAcesso": "23NcT000115501"
*     }
*   ],
*    "dadosCambiais": {
*      "coberturaCambial": {
...
*    ],
    ...
    "tributos": [
     ...
     JsonDUIMP.AddPair('tipoDeclaracao', 'DUIMP');
     JsonDUIMP.AddPair('modalidadeDespacho', 'IM');
     JsonDUIMP.AddPair('canal', 'VERDE');

     cJsonDUIMP.lines.add(JsonDUIMP.Value);
     }
     (*
     with Dados do begin
          with tEmpresas do begin
          ...
          try
            ...
            cJsonDUIMP.Lines.Text := Duimp.ToJsonDUIMP.Format(2);
          finally
            Duimp.Free;
          end;
          }
...
procedure TProcesso_RegistroDI.bPesqMoe1Click(Sender: TObject);
begin
      PesquisaGerais         := TPesquisaGerais.Create(Self);
...
procedure TProcesso_RegistroDI.bPesqMoe2Click(Sender: TObject);
begin
      PesquisaGerais         := TPesquisaGerais.Create(Self);
      PesquisaGerais.mTab    := 'Moedas';
...
procedure TProcesso_RegistroDI.CalculaImpostos;
var
   mvlrFrete
  ,mvlrSeg: Real;
begin
     with Dados, dmDespacho, dmFiscal do begin
          ...
          with tTotal do begin
               ...
               parambyname('pDI').AsString      := DI.FieldByName('DI').AsString;
               parambyname('pAdicao').Asinteger := tAdicoes.FieldByName('Adicao').asinteger;
...
procedure TProcesso_RegistroDI.cProcessoExit(Sender: TObject);
begin
     with Dados, dmDespacho do begin
          if (DI.State = dsInsert) or (DI.State = dsEdit)  then begin
             DIPais_Procedencia.Value      := InvoicePais_Origem.asstring;
             ...
             DIFatura_Numero.Value         := InvoiceNumero.Value;
          end;
     end;
end;

procedure TProcesso_RegistroDI.CriaObjetoJson(pPai: TJSONObject;const pNome: string;const pCampos: array of string;const pValores: array of string);
var
  Obj: TJSONObject;
  i: Integer;
begin
  if Length(pCampos) <> Length(pValores) then raise Exception.Create('Campos e valores com tamanhos diferentes');
  Obj := TJSONObject.Create;
  for i := Low(pCampos) to High(pCampos) do Obj.AddPair(pCampos[i], pValores[i]);
  pPai.AddPair(pNome, Obj);
end;
