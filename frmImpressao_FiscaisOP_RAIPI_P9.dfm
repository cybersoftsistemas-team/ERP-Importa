object Impressao_FiscaisOP_RAIPI_P9: TImpressao_FiscaisOP_RAIPI_P9
  Left = 668
  Top = 328
  BorderStyle = bsDialog
  Caption = 'Impressao_FiscaisOP_RAIPI_P9'
  ClientHeight = 196
  ClientWidth = 417
  Color = clBtnFace
  ...
  object RxLabel2: TRxLabel
    ...
    Width = 266
    Height = 18
    Caption = 'Livro Registro de Apura'#231#227'o do IPI (Mod.P9)'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWhite
    Font.Height = -15
    Font.Name = 'Calibri'
  ...
  object tEntradas: TMSQuery
    ...
    SQL.Strings = (
      ...
      '       SUM(BCIPI) AS Valor_BCIPI, '
      '       SUM(Valor_TotalIPI) AS Valor_IPI,'
      '       SUM(Valor_IsentasIPI) AS Valor_Isentas, '
      '       SUM(Valor_OutrasIPI) AS Valor_Outras '
      'FROM NotasFiscais'
      ...
      '       Natureza_Codigo, '
      '       SUM(Valor_TotalNota), '
      '       SUM(Valor_TotalBCIPI), '
      '       SUM(Valor_TotalIPI), '
      '       SUM(Valor_IsentasIPI), '
      '       SUM(Valor_OutrasIPI)'
      'FROM NotasTerceiros'
    ...
    object tEntradasValor_BCIPI: TCurrencyField
      FieldName = 'Valor_BCIPI'
      Origin = '.'
      ReadOnly = True
    end
    object tEntradasValor_IPI: TCurrencyField
      FieldName = 'Valor_IPI'
      Origin = '.'
      ReadOnly = True
    end
    ...
    object tEntradasValor_Isentas: TFloatField
  ...
  object tEstados: TMSQuery
    ...
    SQL.Strings = (
        ...
        'SELECT SUM(Valor_TotalNota) AS Valor_Contabil, SUM(Valor_TotalPr' +
        'odutos) AS Valor_BCIPI, SUM(Valor_TotalPRodutos) AS Valor_IPI,'
      
        '       0.00 AS Valor_Isentas, SUM(Valor_TotalNota-Valor_TotalPro' +
        'dutos) AS Valor_Outras'
      'FROM NotasFiscais'
  ...
  object rApuracaoIPI: TppReport
    AutoStop = False
    DataPipeline = pEntradas
    NoDataBehaviors = [ndBlankReport]
    PassSetting = psTwoPass
    PrinterSetup.BinName = 'Default'
    PrinterSetup.DocumentName = 'Registro de Apura'#231#227'o de IPI'
    ...
    Template.FileName = 
      'C:\Sistemas\Faturamento\Codigo Fonte\Relatorios\LivroApuracao_IP' +
      'I.rtm'
    AfterPrint = rApuracaoIPIAfterPrint
    ...
    object ppHeaderBand1: TppHeaderBand
      ...
      object ppLabel1: TppLabel
        ...
        AutoSize = False
        Border.mmPadding = 0
        Caption = 'REGISTRO DE APURA'#199#195'O DE IPI'
        Color = clBlack
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWhite
        Font.Name = 'Calibri'
      ...
      object lMovimento: TppLabel
        ...
        BandType = 0
        LayerName = Foreground1
        RotatedOriginLeft = -57692
        RotatedOriginTop = 132654
      end
    end
      ...
      object ppSubReport1: TppSubReport
        ...
        object ppChildReport1: TppChildReport
          ...
          object ppDetailBand2: TppDetailBand
            ...
            object pTotalDebitos: TppDBCalc
              DesignLayer = ppDesignLayer1
            ...
            object pOutrosCreditos: TppDBCalc
              DesignLayer = ppDesignLayer1
  ...
  object pEntradas: TppDBPipeline
    ...
    Left = 48
    Top = 128
    MasterDataPipelineName = 'pApuracao'
  end
  ...
  object tApuracao: TMSQuery
    ...
    SQL.Strings = (
        ...
        'SELECT  Debito_Imposto  = (SELECT ISNULL(SUM(Valor_TotalIPI), 0)' +
        ' FROM NotasFiscais     WHERE Saida_Entrada = 1 AND Cancelada <> ' +
        '1 AND MONTH(Data_Emissao) = 8 AND YEAR(Data_Emissao) = 2014)'
      
        '       ,Outros_Debitos  = (SELECT ISNULL(SUM(Valor), 0)         ' +
        ' FROM OutrosValoresIPI WHERE Debito_Credito = 0 AND MONTH(Data) ' +
        '= 8 AND YEAR(Data) = 2014) +'
      
        '                          (SELECT ISNULL(SUM(Valor), 0)         ' +
        ' FROM Estorno          WHERE Imposto = '#39'IPI'#39' AND Tipo_Ajuste IN(' +
        '0, 5) AND (MONTH(Data) = 8) AND (YEAR(Data) = 2014))'
      
        '       ,Estorno_Credito = (SELECT ISNULL(SUM(Valor), 0)         ' +
        ' FROM Estorno          WHERE MONTH(Data) = 8 AND YEAR(Data) = 20' +
        '14 AND Tipo_Ajuste = 1 AND Imposto = '#39'IPI'#39')'
      
        '       ,Outros_Creditos = (SELECT ISNULL(SUM(Valor), 0)         ' +
        ' FROM Estorno          WHERE MONTH(Data) = 8 AND YEAR(Data) = 20' +
        '14 AND Tipo_Ajuste = 2 AND Imposto = '#39'IPI'#39')'
      
        '       ,Estorno_Debito  = (SELECT ISNULL(SUM(Valor), 0)         ' +
        ' FROM Estorno          WHERE MONTH(Data) = 8 AND YEAR(Data) = 20' +
        '14 AND Tipo_Ajuste = 3 AND Imposto = '#39'IPI'#39')'
      
        '       ,Deducoes        = (SELECT ISNULL(SUM(Valor), 0)         ' +
        ' FROM Estorno          WHERE MONTH(Data) = 8 AND YEAR(Data) = 20' +
        '14 AND Tipo_Ajuste = 4 AND Imposto = '#39'IPI'#39')'
      
        '       ,Saldo_Anterior  = ISNULL((SELECT Saldo FROM SaldoIPI WHE' +
        'RE Mes_Ano = '#39'082014'#39'), 0)')
    FetchRows = 1
    Left = 216
    Top = 41
  end
