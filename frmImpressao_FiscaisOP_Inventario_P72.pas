Unit frmImpressao_FiscaisOP_Inventario_P72;

interface

uses
  Winapi.Windows, Winapi.Messages, System.SysUtils, System.Variants, System.Classes, Vcl.Graphics, Vcl.Controls, Vcl.Forms, Vcl.Dialogs, DB, DBAccess, MSAccess, RxLookup, Vcl.StdCtrls, Mask, RXSpin,
  Vcl.ExtCtrls, RXCtrls, Buttons, INIFiles, Funcoes, MaskUtils, DBCtrls, ppCtrls, ppPrnabl, ppClass, ppBands,
  ppReport, ppDBPipe, ppMemo, ppVar, ComObj,
  RxToolEdit, ppDB, ppParameter, ppDesignLayer, ppStrtch, ppCache, ppComm, ppRelatv, ppProd, MemDS;

type
  TImpressao_FiscaisOP_Inventario_P72 = class(TForm)
    Panel1: TPanel;
    bSair: TButton;
    bImprimir: TButton;
    cData: TDateEdit;
...
var
  Impressao_FiscaisOP_Inventario_P72: TImpressao_FiscaisOP_Inventario_P72;
...
procedure TImpressao_FiscaisOP_Inventario_P72.bSairClick(Sender: TObject);
begin
        Close;
end;

procedure TImpressao_FiscaisOP_Inventario_P72.FormShow(Sender: TObject);
Var
   aINI : TIniFile;
begin
      Screen.Cursor := crSQLWait;
      Impressao_FiscaisOP_Inventario_P72.Color := $00FFE6B0;
      ...
      cImpressora.ItemIndex := aINI.ReadInteger('IMPRESSAO_Inventario','Impressora', 0);
...
procedure TImpressao_FiscaisOP_Inventario_P72.FormClose(Sender: TObject;var Action: TCloseAction);
Var
   aINI : TIniFile;
begin
      ...
      LimpaMemoria;
      Impressao_FiscaisOP_Inventario_P72.Release;
      Impressao_FiscaisOP_Inventario_P72 := nil;
end;

procedure TImpressao_FiscaisOP_Inventario_P72.ModeloP7;
Var
...
procedure TImpressao_FiscaisOP_Inventario_P72.bImprimirClick(Sender: TObject);
begin
      Screen.Cursor := crSQLWait;
      ...
      With dmFiscal, Dados do begin
           ...
           If EmpresasDistribuidora.AsBoolean = false then begin
              ...
              Inventario.SQL.Add('                         AND Data<= :pDataSaida)');
              ...
              Inventario.SQL.Add('       ,Unidade');
              Inventario.SQL.Add('       ,ROUND((');
              Inventario.SQL.Add('        ISNULL((SELECT SUM(Quantidade) FROM NotasTerceirosItens WHERE(Codigo_Mercadoria = Codigo) AND (Movimenta_Inventario = 1) AND ((SELECT Finalidade_Mercadoria FROM ReferenciasFiscais WHERE(Codigo = Referencia_Fiscal)) = 5)');
              Inventario.SQL.Add('               AND (Data_Entrada <= :pData)), 0) +');
              Inventario.SQL.Add('        ISNULL((SELECT SUM(Quantidade) FROM NotasItens WHERE(Saida_Entrada = 0) AND (Codigo_Mercadoria = Codigo) AND (Movimenta_Inventario = 1) AND (Cancelada <> 1) AND (ISNULL(Nfe_Denegada, 0) = 0) AND (Data <= :pData)');
              Inventario.SQL.Add('                AND Finalidade_Mercadoria = 5), 0) -');
              Inventario.SQL.Add('        ISNULL((SELECT SUM(Quantidade) FROM NotasItens WHERE(Saida_Entrada = 1) AND (Codigo_Mercadoria = Codigo) AND (Movimenta_Inventario = 1) AND (Cancelada <> 1) AND (ISNULL(Nfe_Denegada, 0) = 0) AND (Data <= :pDataSaida)');
              Inventario.SQL.Add('                AND Finalidade_Mercadoria = 5), 0)');
              Inventario.SQL.Add('       ), 3) AS Quantidade');
              If cData.Date < StrtoDate('01/01/2014') then begin
                 Inventario.SQL.Add('       ,Valor_Unitario = ((SELECT SUM(ISNULL(Valor_Total,0)+ISNULL(Valor_Despesa*Quantidade,0)) FROM NotasItens WHERE Codigo_Mercadoria=Codigo AND (Movimenta_Inventario = 1) AND Saida_Entrada=0 AND Cancelada<>1');
                 Inventario.SQL.Add('                         AND ISNULL(Nfe_Denegada, 0)=0 AND Movimenta_Inventario=1 AND Data<=:pData))');
                 Inventario.SQL.Add('                          +(SELECT SUM(ISNULL(Valor_Inventario,0)) FROM NotasTerceirosItens WHERE Codigo_Mercadoria = Codigo AND (Movimenta_Inventario = 1) AND Inventario_Terceiros = 0 AND Data_Entrada<=:pData');
                 Inventario.SQL.Add('                          AND (SELECT Finalidade_Mercadoria FROM ReferenciasFiscais WHERE Codigo = Referencia_Fiscal) IN(0, 2, 3, 5, 6))');
           ...
           If EmpresasDistribuidora.AsBoolean = true then begin
              ...
              Inventario.SQL.Add('        ISNULL((SELECT SUM(Quantidade) FROM NotasTerceirosItens WHERE(Codigo_Mercadoria = Codigo) AND Movimenta_Inventario=1 AND ((SELECT Finalidade_Mercadoria FROM ReferenciasFiscais WHERE(Codigo = Referencia_Fiscal)) = 5)');
              Inventario.SQL.Add('        AND (Data_Entrada <= :pData)), 0) +');
              Inventario.SQL.Add('        ISNULL((SELECT SUM(Quantidade) FROM NotasItens WHERE(Saida_Entrada = 0) AND (Codigo_Mercadoria = Codigo) AND Movimenta_Inventario=1 AND (Cancelada <> 1) AND (ISNULL(Nfe_Denegada, 0) = 0) AND (Movimenta_Inventario = 1) AND (Data <= :pData)');
              Inventario.SQL.Add('                AND Finalidade_Mercadoria = 5), 0) -');
              Inventario.SQL.Add('        ISNULL((SELECT SUM(Quantidade) FROM NotasItens WHERE(Saida_Entrada = 1) AND (Codigo_Mercadoria = Codigo) AND Movimenta_Inventario=1 AND (Cancelada <> 1) AND (ISNULL(Nfe_Denegada, 0) = 0) AND (Movimenta_Inventario = 1) AND (Data <= :pDataSaida)');
              Inventario.SQL.Add('                AND Finalidade_Mercadoria = 5), 0)');
              Inventario.SQL.Add('       ), 3) AS Quantidade,');
              Inventario.SQL.Add('       (');
              Inventario.SQL.Add('        ISNULL((SELECT SUM(Valor_Liquido) FROM NotasTerceirosItens WHERE(Codigo_Mercadoria = Codigo) AND Movimenta_Inventario=1 AND ((SELECT Finalidade_Mercadoria FROM ReferenciasFiscais WHERE(Codigo = Referencia_Fiscal)) = 5)');
              Inventario.SQL.Add('                AND (Data_Entrada <= :pData)), 0) +');
              Inventario.SQL.Add('        ISNULL((SELECT SUM(Valor_Inventario*Quantidade) FROM NotasItens WHERE(Saida_Entrada = 0) AND (Codigo_Mercadoria = Codigo) AND Movimenta_Inventario=1 AND (Cancelada <> 1) AND (ISNULL(Nfe_Denegada, 0) = 0)');
              Inventario.SQL.Add('                AND (Movimenta_Inventario = 1) AND (Data <= :pData)');
              Inventario.SQL.Add('                AND Finalidade_Mercadoria = 5), 0) -');
              Inventario.SQL.Add('        ISNULL((SELECT SUM(Valor_Inventario*Quantidade) FROM NotasItens WHERE(Saida_Entrada = 1) AND (Codigo_Mercadoria = Codigo) AND Movimenta_Inventario=1 AND (Cancelada <> 1) AND (ISNULL(Nfe_Denegada, 0) = 0)');
              Inventario.SQL.Add('                AND (Movimenta_Inventario = 1) AND (Data <= :pData)');
              Inventario.SQL.Add('        AND Finalidade_Mercadoria = 5), 0)');
              Inventario.SQL.Add('       ) / ');
              ...
              Inventario.SQL.Add('                AND (Data_Entrada <= :pData)), 0) +');
              Inventario.SQL.Add('        ISNULL((SELECT SUM(Quantidade) FROM NotasItens          WHERE(Saida_Entrada = 0) AND (Codigo_Mercadoria = Codigo) AND Movimenta_Inventario=1 AND (Cancelada <> 1) AND (ISNULL(Nfe_Denegada, 0) = 0) AND (Movimenta_Inventario = 1) AND (Data <= :pData)');
              Inventario.SQL.Add('                       AND (SELECT Finalidade_Mercadoria FROM TipoNota WHERE(Codigo = Tipo_Nota)) = 5), 0) -');
              Inventario.SQL.Add('        ISNULL((SELECT SUM(Quantidade) FROM NotasItens          WHERE(Saida_Entrada = 1) AND (Codigo_Mercadoria = Codigo) AND Movimenta_Inventario=1 AND (Cancelada <> 1) AND (ISNULL(Nfe_Denegada, 0) = 0) AND (Movimenta_Inventario = 1) AND (Data <= :pData)');
              Inventario.SQL.Add('                       AND (SELECT Finalidade_Mercadoria FROM TipoNota WHERE(Codigo = Tipo_Nota)) = 5), 0)');
...
procedure TImpressao_FiscaisOP_Inventario_P72.FormCreate(Sender: TObject);
begin
      If FileExists('fundo_barra.bmp') then Image1.Picture.LoadFromFile('fundo_barra.bmp');
end;

procedure TImpressao_FiscaisOP_Inventario_P72.rInventarioBeforePrint(Sender: TObject);
begin
      //lPeriodo.Caption := cData.Text;
      lPeriodo.Caption := cDataSaida.Text;
end;

procedure TImpressao_FiscaisOP_Inventario_P72.ppDetailBand1BeforePrint(Sender: TObject);
begin
...
procedure TImpressao_FiscaisOP_Inventario_P72.ppHeaderBand1BeforePrint(Sender: TObject);
begin
      lFolha.Caption := InttoStr( cPaginaIni.AsInteger + (rInventario.PageNo)-1);
end;

procedure TImpressao_FiscaisOP_Inventario_P72.ppGroupFooterBand1BeforePrint(Sender: TObject);
begin
...
procedure TImpressao_FiscaisOP_Inventario_P72.ppGroupHeaderBand1BeforePrint(Sender: TObject);
begin
     If dmFiscal.InventarioInventario.AsString = 'P' then begin
...
procedure TImpressao_FiscaisOP_Inventario_P72.ppSummaryBand1BeforePrint(Sender: TObject);
begin
...
procedure TImpressao_FiscaisOP_Inventario_P72.cCodigoChange(Sender: TObject);
begin
      Try
...
procedure TImpressao_FiscaisOP_Inventario_P72.ExportaEXCEL;
var
   mPlanilha      : Variant;
   mLinha         : Integer;
   mTotalQtdePro  : Real;
...
procedure TImpressao_FiscaisOP_Inventario_P72.cNegativoClick(Sender: TObject);
begin
      If cNegativo.Checked = true then cPositivo.Checked := false;
end;

procedure TImpressao_FiscaisOP_Inventario_P72.cPositivoClick(Sender: TObject);
begin
      If cPositivo.Checked = true then cNegativo.Checked := false;
end;

procedure TImpressao_FiscaisOP_Inventario_P72.cDataExit(Sender: TObject);
begin
      If DataLimpa(cDataSaida.Text) then cDataSaida.Date := cData.Date;
end;
