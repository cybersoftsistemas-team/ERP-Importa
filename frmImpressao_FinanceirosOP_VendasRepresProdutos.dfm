object Impressao_FinanceirosOP_VendasRepresProdutos: TImpressao_FinanceirosOP_VendasRepresProdutos
  Left = 282
  Top = 283
  BorderStyle = bsDialog
  Caption = 'Impressao_FinanceirosOP_VendasRepresProdutos'
  ClientHeight = 202
  ClientWidth = 863
  Color = clBtnFace
  ...
  object RxLabel2: TRxLabel
    ...
    Width = 247
    Height = 18
    Caption = 'Vendas de representantes por produtos.'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWhite
    Font.Height = -15
    Font.Name = 'Calibri'
  ...
  object tItens: TMSQuery
    ...
    SQL.Strings = (
      'SELECT Representante AS Codigo,'
      
        '       (SELECT Nome FROM Fornecedores WHERE(Codigo = NF.Represen' +
        'tante)) AS Nome,'
      '       NI.Codigo_Mercadoria AS Produto,'
      
        '       CAST(NI.Descricao_Mercadoria AS VARCHAR(500)) AS Descrica' +
        'o,'
      '       NI.Valor_Total,'
      '       Quantidade'
      'INTO #TEMP'
      'FROM   NotasFiscais NF, NotasItens NI'
      'WHERE (NF.Numero = NI.Nota AND NF.Data_Emissao = NI.Data) '
      
        '      AND (NF.Data_Emissao BETWEEN '#39'01/01/2012'#39' AND '#39'07/11/2012'#39 +
        ') '
      '      AND (NF.Cancelada <> 1)'
      '      AND (NF.Representante = 7) '
      'ORDER BY Nome, Codigo_Mercadoria'
      ''
      'SELECT Codigo, '
      '       Nome,'
      '       Produto,'
      
        '       (SELECT REPLACE(Descricao, '#39'<{'#39'+LTRIM(RTRIM(CAST(Produto ' +
        'AS VARCHAR(15))))+'#39'}>'#39', '#39#39') FROM Produtos WHERE(Codigo = Produto' +
        ')) AS Descricao,       '
      '       SUM(Valor_Total) AS Valor,'
      '       SUM(Quantidade) AS Quantidade'
      'FROM #TEMP'
      'GROUP BY Codigo, Nome, Produto'
      'DROP TABLE #TEMP')
    FetchRows = 1
    Active = True
    Left = 488
  ...
  object rVendasRepresProduto: TppReport
    AutoStop = False
    DataPipeline = pItens
    NoDataBehaviors = [ndBlankReport]
    PassSetting = psTwoPass
    PrinterSetup.BinName = 'Default'
    PrinterSetup.DocumentName = 'Total de vendas de representantes - Por Produtos'
    ...
    object ppHeaderBand2: TppHeaderBand
      ...
      object lTitulo: TppLabel
        ...
        AutoSize = False
        Border.mmPadding = 0
        Caption = 'Total das vendas de representantes por produtos'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 10
    ...
    object ppGroup1: TppGroup
      ...
      object gRodapeRepresentante: TppGroupFooterBand
        Background.Brush.Style = bsClear
        Border.mmPadding = 0
