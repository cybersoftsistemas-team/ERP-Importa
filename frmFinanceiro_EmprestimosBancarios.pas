unit frmFinanceiro_EmprestimosBancarios;
...
uses
  ...
  Funcoes, RXDBCtrl, RxDBComb, DB, MemDS, DBAccess, MSAccess, Grids, DBGrids, RXSpin, DateUtils,
  ComCtrls, RxCurrEdit;

type
  TFinanceiro_EmprestimosBancarios = class(TForm)
    Panel1: TPanel;
    bSair: TButton;
    XPPanel1: TXPPanel;
    Image1: TImage;
...
var
  Financeiro_EmprestimosBancarios: TFinanceiro_EmprestimosBancarios;
...
procedure TFinanceiro_EmprestimosBancarios.FormCreate(Sender: TObject);
begin
      If FileExists('fundo_barra.bmp') then XPPanel1.BgImage.LoadFromFile('fundo_barra.bmp');
end;

procedure TFinanceiro_EmprestimosBancarios.bSairClick(Sender: TObject);
begin
      Close;
end;

procedure TFinanceiro_EmprestimosBancarios.FormClose(Sender: TObject;var Action: TCloseAction);
begin
      FecharTabelas(Dados,nil,nil,nil);
      Financeiro_EmprestimosBancarios.Release;
      Financeiro_EmprestimosBancarios := nil;
end;

procedure TFinanceiro_EmprestimosBancarios.FormShow(Sender: TObject);
begin
...
procedure TFinanceiro_EmprestimosBancarios.NavegaClick(Sender: TObject;Button: TNavigateBtn);
Var
   i: Integer;
begin
...
procedure TFinanceiro_EmprestimosBancarios.NavegaBeforeAction(Sender: TObject; Button: TNavigateBtn);
begin
      With Dados do begin
           ...
           If (Button = nbPost) and (Emprestimos.State = dsInsert) then begin
              tNumero.SQL.Clear;
              tNumero.SQL.Add('SELECT MAX(Numero) AS Numero FROM Emprestimos');
              tNumero.Open;
              EmprestimosNumero.Value           := tNumero.FieldByName('Numero').AsInteger + 1;
              EmprestimosNumero_Documento.Value := EmprestimosTipo.AsString + EmprestimosNumero.AsString;
              tNumero.Close;
           End;
...
procedure TFinanceiro_EmprestimosBancarios.cProcessoChange(Sender: TObject);
begin
      With Dados do begin
...
procedure TFinanceiro_EmprestimosBancarios.bParcelasClick(Sender: TObject);
begin
      With Dados do begin
...
procedure TFinanceiro_EmprestimosBancarios.cBancoClienteChange(Sender: TObject);
begin
      With Dados do begin
...
procedure TFinanceiro_EmprestimosBancarios.Parcelas;
var
    mQtde     : Integer;
    mData     : TDate;
    mUltimoDia: Integer;
...
procedure TFinanceiro_EmprestimosBancarios.cValorPrincipalChange(Sender: TObject);
begin
      cTotal.Value := Dados.EmprestimosValor_Principal.AsCurrency + Dados.EmprestimosValor_Juros.AsCurrency;
end;

procedure TFinanceiro_EmprestimosBancarios.cValorJurosChange(Sender: TObject);
begin
      cTotal.Value := Dados.EmprestimosValor_Principal.AsCurrency + Dados.EmprestimosValor_Juros.AsCurrency;
end;

procedure TFinanceiro_EmprestimosBancarios.cCodigoChange(Sender: TObject);
begin
      With Dados do begin
...
procedure TFinanceiro_EmprestimosBancarios.bFinanceiroClick(Sender: TObject);
begin
      // Verifica se ja houve baixas para alguma parcela do emprestimo.
      tNumero.SQL.Clear;
      tNumero.SQL.Add('SELECT COUNT(*) AS Qtde FROM PagarReceberBaixas WHERE Numero IN( SELECT Financeiro_Numero FROM EmprestimosParcelas EP WHERE EP.Emprestimo = :pNumero )');
...
procedure TFinanceiro_EmprestimosBancarios.GradeParcelasDrawColumnCell(Sender: TObject; const Rect: TRect; DataCol: Integer; Column: TColumn;State: TGridDrawState);
var
    Icon: TBitmap;
begin
      With Dados do begin
