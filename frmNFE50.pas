...
procedure TNFE50.bSairClick(Sender: TObject);
begin
      Close;
end;

procedure TNFE50.FormClose(Sender: TObject;var Action: TCloseAction);
Var
   aINI:TIniFile;
begin
...
procedure TNFE50.FormShow(Sender: TObject);
Var
   aINI:TIniFile;
begin
      ...
      xmlProtocolo         := TMemo.Create(NFE50);
      xmlProtocolo.Visible := False;
      xmlProtocolo.Parent  := NFE50;
      Screen.Cursor        := crDefault;
end;

procedure TNFE50.bEnviarClick(Sender: TObject);
Var
    mSel    : Integer;
    mParcela: Integer;
begin
       ...
       With Dados, dmFiscal do begin
            ...
            For mSel := 0 to Grade.SelectedRows.Count-1 do begin
                ...
                RegistraPasso('1.0');
                cMensagens.Lines.Add('Gerando o arquivo XML.');
                GerarXML;                                  
                RegistraPasso('1.1');
                ...
                If mQtdErros = 0 then begin
                   ...
                   if ide_tpEmis in[1, 6, 7] then begin
                      ...
                      if ide_tpEmis in[6, 7] then begin
                         cMensagens.Lines.Add('Verificando se servidor de contigência esta ativo.');
                         if not ConsultaContig then Abort;
                      end;
                      if Pedidos.FieldByName('Aguardando').AsBoolean = false then begin
                         if ide_tpEmis = 1 then
                             cMensagens.Lines.Add('Enviando o arquivo XML das notas para SEFAZ (Normal).')
                         else
                             cMensagens.Lines.Add('Enviando o arquivo XML das notas para SEFAZ (Contigência SVC)');
                         Application.ProcessMessages;
                         EnviarNFE;
                      end else begin
                         cMensagens.Lines.Add('Arquivo XML da nota já enviado anteriormente.');
                      end;
                      RegistraPasso('2.1');
                   end;
                   ...
                   if ide_tpEmis = 4 then begin
                      mQtdErros := 0;
                      cMensagens.Lines.Add('Enviando o arquivo XML da EPEC para SEFAZ.');
                      EnviarEPEC;
                      RegistraPasso('3.1');
                      if mQtdErros > 0 then begin
                ...
                If mQtdErros = 0 then begin
                   ...
                   RegistraPasso('6.0');
                   cMensagens.Lines.Add('Salvando a nota fiscal enviada no banco de dados.');
                   Application.ProcessMessages;
                   ...
                   RegistraPasso('7.0');
                   cMensagens.Lines.Add('Criando o arquivo PDF do DANFE...');
                   ...
                   if not TipoNotaEmail_NFE.AsBoolean then begin
                      if (Pedidos.FieldByName('Saida_Entrada').AsInteger = 1) and (ide_tpEmis in[1, 6, 7]) and (not Clientes.FieldByName('Enviar_Email').AsBoolean) then begin
                         RegistraPasso('7.2');
                         cMensagens.Lines.Add('Enviando o "XML" e o "PDF" do DANFE da NF-e para o(s) E-Mail(s): ');
                   ...
                   if ConfiguracaoEnviarNFe_API.AsBoolean then begin
                      RegistraPasso('7.3');
                      cMensagens.Lines.Add('Enviando o "XML" da NF-e para API DOMÍNIO (Thomson Reuters)');
                      FAPI.enviaXMLAPI;
                   end;
...
//                                tProvisao.Connection    := Banco_Empresas;
...
begin
       Screen.Cursor    := crSQLWait;
       Util             := CoUtil.Create;
       mXMLTemp         := TMemo.Create(NFE50);
       mXMlTemp.Visible := False;
       mXMLTemp.Parent  := NFE50;
       ...
       With Dados, dmFiscal do begin
            ...
            If (not PedidosComplementar.AsBoolean) and (not PedidosAjuste.AsBoolean) then ide_finNFe := 1;
            If PedidosComplementar.AsBoolean then ide_finNFe := 2;
            If PedidosAjuste.AsBoolean       then ide_finNFE := 3;
            If PedidosDevolucao.AsBoolean    then ide_finNFE := 4;
            If PedidosNFE_Estorno.AsBoolean  then ide_finNFE := 3;
            ...
            ide := Util.identificadorRTC(ide_cUF                                                    // 01. Código da UF do emitente do Documento Fiscal, utilizar a codificação do IBGE (Ex. SP->35, RS->43, etc.
                                        ,ide_cNF                                                    // 02. Código numérico que compõe a Chave de Acesso. Número aleatório gerado pelo emitente para cada NF-e para evitar acessos indevidos da NF-e.
                                        ,ide_NatOP                                                  // 03. Natureza da operação de que decorrer a saída ou a entrada, tais como: venda, compra, transferência, devolução, importação, consignação, remessa (para fins de demonstração, de industrialização outra).
                                        ,ide_Mode                                                   // 04. Código do Modelo do Documento Fiscal, código 55 para a NF-e ou código 65 para NFC-e.
                                        ,ide_Serie                                                  // 05. Série do Documento Fiscal, preencher com zeros na hipótese de a NF-e não possuir série.
                                        ,ide_nNF                                                    // 06. Número do Documento Fiscal.
                                        ,ide_dEmi310                                                // 07. Data de emissão do Documento Fiscal no padrão UTC - Universal Coordinated Time
                                        ,ide_dSaiEnt310                                             // 08. Data e hora de Saída ou da Entrada da Mercadoria/Produto no padrão UTC - Universal Coordinated Time.
                                        ,ide_tpNF                                                   // 09. Código do tipo do Documento Fiscal:0 - entrada / 1 - saída.
                                        ,ide_Dest                                                   // 10. Identificador de local de destino da operação: 1-Operação interna / 2-Operação interestadual /3-Operação com exterior.
                                        ,ide_cMunFG                                                 // 11. Código do Município de Ocorrência do Fato Gerador do ICMS, que é o local onde ocorre a entrada ou saída da mercadoria, utilizar a Tabela do IBGE.
                                        ,ide_NFRef                                                  // 12. Grupo de informações NFRef dos documentos fiscais referenciados, Utilizar para referenciar um Documento Fiscal vinculado a NF-e atual.
                                        ,ide_tpImp                                                  // 13. Formato de impressão do DANFE:0-Sem geração de DANFE/1-DANFE normal, Retrato/2-DANFE normal, Paisagem/3-DANFE Simplificado/4-DANFE NFC-e/5-DANFE NFC-e em mensagem eletrônica.
                                        ,iif(not PedidosDPEC.asboolean, InttoStr(ide_tpEmis), 4)    // 14. Código da forma de emissão:1-Emissão normal/2-Contingência FS-I/3-Contingência SCAN/4-Contingência EPEC/5-Contingência FS-DA/6-Contingência SVC-AN/7-Contingência SVC-RS/9-Contingência off-line da NFC-e.
                                        ,ide_cDV                                                    // 15. Código do dígito verificador - DV da Chave de Acesso da NF-e, o DV será calculado com a aplicação do algoritmo módulo 11 (base 2,9) da Chave de Acesso.
                                        ,ide_tpAmb                                                  // 16. Código de identificação do Ambiente:1 - Produção / 2 - Homologação.
                                        ,ide_finNFe                                                 // 17. Código da finalidade de emissão da NF-e:1-NF-e normal / 2-NF-e complementar /3-NF-e de ajuste / 4-Devolução / 5-Nota de crédito / 6-Nota de débito
                                        ,ide_Final                                                  // 18. Indicador de operação com Consumidor final:0-Não / 1-Consumidor final.
                                        ,ide_Pres                                                   // 19. Indicador de presença do comprador no estabelecimento comercial no momento da operação.
                                        ,ide_procEmi                                                // 20. Código de identificação do processo de emissão da NF-e: Identificador do processo de emissão da NF-e.
                                        ,ide_verProc                                                // 21. Versão do processo de emissão da NF-e utilizada (aplicativo emissor de NF-e).
                                        ,''                                                         // 22. Data e hora de entrada em contingência no padrão UTC.
                                        ,''                                                         // 23. Justificativa de entrada em contingência, deve ser informado sempre que tpEmis for diferente de 1.
                                        ,ide_Interm                                                 // 24. Indicador de intermediador/marketplace.
                                        ,''                                                         // 25. Código do Município de Ocorrência do Fato Gerador do IBS/CBS.
                                        ,''                                                         // 26. Tipo de Nota de Débito:01=Transferência de créditos para Cooperativas/02=Anulação de Crédito por Saídas Imunes/Isentas/03=Débitos de notas fiscais não processadas na apuração/04=Multa e juros/05=Transferência de crédito de sucessão.
                                        ,''                                                         // 27. Tipo de Nota de Crédito:A Definir.
                                        ,''                                                         // 28. Grupo gCompraGov - Compra Governamental.
                                        ,''                                                         // 29. Chave(s) das NFe(s) gPagAntecipado - Notas de antecipação de pagamentos.
                                        );
            ...
            totICMS := Util.totalICMSNT2023001(PedidosBCICMS.AsCurrency,                                                       //01 somatório da BC do ICMS (vBC) informado nos itens.
                                               ...
                                               iif(PedidosSaida_Entrada.Value = 0, PedidosValor_TotalII.AsFloat, 0),           //09 somatório de II (vII) informado nos itens.
                                               ...
                                               PedidosValor_FCP.AsCurrency,                                                    //19 somatório do Valor do FCP (Fundo de Combate à Pobreza) (vFCP) informado nos itens.
                                               iif(PedidosDIFAL_Valor.Value > 0 , 0, PedidosValor_FCP.Value),                  //20 somatório do Valor do FCP retido anteriormente por Substituição. Corresponde ao total da soma dos campos vFCPST informado nos itens.
                                               PedidosValor_FCPST.AsCurrency,                                                  //21 somatório do Valor do FCP retido anteriormente por Substituição. Corresponde ao total da soma dos campos vFCPSTRet informado nos itens.
                                               0,                                                                              //22 informar o somatório do Valor do FCP retido anteriormente por Substituição. Corresponde ao total da soma dos campos vFCPSTRet informado nos itens.
                                               PedidosValor_IPIDevol.ascurrency,                                               //23 somatório do Valor do IPI devolvido.
            ...
            *)
            ...
            mAutCNPJ         := TMemo.Create(NFE50);
            mAutCNPJ.Visible := False;
            mAutCNPJ.Parent  := NFE50;
            mAutCPF          := TMemo.Create(NFE50);
            mAutCPF.Visible  := False;
            mAutCPF.Parent   := NFE50;
...
Function TNFE50.MontaDetalhe(item:integer; i:Integer; Qtde:double; Valor:double): string;
var
   Util: NFe_Util_2G_Interface;
   _produto,
   _DI,
      ...
      With Dados, dmFiscal do Begin
           ...
           if Adicoes.Locate('Codigo_Mercadoria', PedidosItensCodigo_Mercadoria.Value, [loCaseInsensitive]) then begin
              if (Trim(ProcessosDOCTipo.Value) = 'IMPORTAÇÃO') and (ProcessosDOCCourrier.AsBoolean = false) then begin
                 if Trim(PedidosDI.AsString) <> '' then begin
           ...
           if PedidosItensValor_ICMSPresumido.Ascurrency > 0 then begin
              ...
              _CredPresumido := Util.CredPresumido(PedidosItensCodigo_CredPres.AsString          // 01.Informar o Código de Benefício Fiscal de Crédito Presumido utilizado pela UF.
                                                  ,PedidosItensAliquota_ICMSPresumido.AsFloat    // 02.Informar o percentual do crédito presumido relativo ao código do crédito presumido informado.
                                                  ,PedidosItensValor_ICMSPresumido.Ascurrency    // 03.Informar o valor do crédito presumido relativo ao código do crédito presumido informado.
                                                  );
              }                                    
              _CredPresumido := Util.CredPresumido(PedidosItensCodigo_CredPres.AsString          // 01.Informar o Código de Benefício Fiscal de Crédito Presumido utilizado pela UF.
           ...
           _produto := Util.produtoRTC(mCodigoProd                                                           // 01.informar o código do produto ou serviço.
                                      ,mGTIN                                                                 // 02.informar o GTIN (Global Trade Item Number) do produto, antigo código EAN ou código de barras.
                                      ,mDescricao                                                            // 03.informar a descrição do produto ou serviço.
                                      ,PedidosItensNCM.Value                                                 // 04.nformar o Código NCM com 8 dígitos.
                                      ,''                                                                    // 05.Codificação NVE-Nomenclatura Valor Aduaneiro e Estatística, Codificação opcional que detalha alguns NCM.
                                      ,PedidosItensCEST.Value                                                // 06.informar Código Especificador da Substituição Tributária - CEST.
                                      ,mEscala                                                               // 07.Indicador de Produção em escala relevante.
                                      ,ProdutosCNPJ_Fabricante.AsString                                      // 08.CNPJ do Fabricante da Mercadoria, obrigatório para produto em escala NÃO relevante.
                                      ,PedidosItensBeneficio_Fiscal.AsString                                 // 09.Código de Benefício Fiscal utilizado pela UF, aplicado ao item.
                                      ,Trim(NCMCodigo_EXTIPI.AsString)                                       // 10.EXTIPI.
                                      ,PedidosItensNatureza_Codigo.AsInteger                                 // 11.Código da CFOP.
                                      ,RemoveCaracterXML(ProdutosUnidade_Origem.Value)                       // 12.informar a unidade de comercialização do produto.
                                      ,FormatFloat('0.0000', mQuantidade)                                    // 13.nformar a quantidade de comercialização do produto.
                                      ,FormatFloat('0.0000000000', PedidosItensValor_Unitario.Value)         // 14.Informar o valor unitário de comercialização do produto.
                                      ,PedidosItensValor_Total.AsCurrency                                    // 15.informar o valor total bruto do produto ou serviços.
                                      ,RemoveCaracterXML(mGTINUnidade)                                       // 16.informar o GTIN (Global Trade Item Number) da unidade de tributação do produto.
                                      ,RemoveCaracterXML(PedidosItensUnidade_Medida.AsString)                // 17.informar a unidade de tributação do produto.
                                      ...
                                      ,1                                                                     // 24.Este campo deverá ser preenchido com: 0 - o valor do item (vProd) não compõe o valor total da NF-e (vProd).
                                      ,_DI                                                                   // 25.informar o XML do grupo DI - dados da importação nas operações de importação.
                                      ,_Exporta                                                              // 26.Detalhamento da exportação.
                                      ,_Especifico                                                           // 27.informar o XML do grupo - detalhamento de específico.
                                      ,PedidosItensPO.AsString                                               // 28.informar o número do pedido de compra, o campo é de livre uso do emissor.
                                      ,PedidosItensOrdem.AsString                                            // 29.informar o número do item do pedido de compra, o campo é de livre uso do emissor.
                                      ,''                                                                    // 30.informar o Número de controle da FCI - Ficha de Conteúdo de Importação com formatação.
                                      ,_lote                                                                 // 31.Número de controle da FCI - Ficha de Conteúdo de Importação.
                                      ,_CredPresumido                                                        // 32.Informar o grupo de Informações do Crédito Presumido.
                                      ,0);                                                                   // 33.nformar o Indicador de fornecimento de bem móvel usado.Somente para fornecimentos de bem móvel usado adquirido de pessoa física que não seja contribuinte ou que seja inscrita como MEI.1 - Bem Móvel Usado [RTC]
           ...
           _II := Util.II(iif(PedidosSaida_Entrada.Value = 0, PedidosItensValor_Total.Value, 0),
                          iif(PedidosSaida_Entrada.Value = 0, mDespesa, 0),
                          Roundto(iif(PedidosSaida_Entrada.Value = 0, PedidosItensValor_II.asfloat * (mQuantidade / ProdutosQuantidade_Unidade.AsFloat), 0), -2),
                          0);
                          
           mAnalise.Lines.Add(floattostr(Roundto(PedidosItensValor_II.asfloat * (mQuantidade / ProdutosQuantidade_Unidade.AsFloat), -2)));

           // Monta as TAGS de impostos.
           ...
           // Monta as TAGS de detalhamento dos produtos.
           _Detalhe := Util.detalheRTC(Item
                                      ,_Produto
                                      ,_Imposto
                                      ,mDescricaoResto
                                      ,iif(PedidosItensValor_IPIDevol.value > 0, 100, 0)
                                      ,PedidosItensValor_IPIDevol.value
                                      ,''
                                      ,''
                                      ,PedidosItensValor_Total.value
                                      ,'');
...
begin
      with Dados, dmFiscal do Begin
           if NFe <> '' then begin
              ...
              MemoAssinado         := TMemo.Create(NFE50);
              MemoAssinado.Visible := False;
              MemoAssinado.Parent  := NFE50;
...
procedure TNFE50.bDANFEClick(Sender: TObject);
begin
      DANFE_Retrato.AllowPrintToFile := True;
...
procedure TNFE50.FormCreate(Sender: TObject);
begin
     FAPI := TServiceAPIDominio.Create;
     If FileExists('fundo_barra.bmp') then Image2.Picture.LoadFromFile('fundo_barra.bmp');
end;

procedure TNFE50.FormDestroy(Sender: TObject);
begin
     if Assigned(FAPI) then FreeAndNil(FAPI);
end;

procedure TNFE50.BuscarNFe;
var
    Util: NFe_Util_2G_Interface;
begin
      With Dados, dmFiscal do begin
...
procedure TNFE50.ppDetailBand2BeforePrint(Sender: TObject);
begin
     lItem_CST.Caption := '';
...
procedure TNFE50.ppHeaderBand1BeforePrint(Sender: TObject);
Var
    mParcela: Integer;
begin
      With Dados, dmFiscal do Begin
...
procedure TNFE50.tTempoTimer(Sender: TObject);
begin
      mProcessamento := False;
      tTempo.Enabled := False;
end;

procedure TNFE50.bSelecionarClick(Sender: TObject);
var
    vlLinha: Integer;
begin
...
procedure TNFE50.bDesmarcarClick(Sender: TObject);
begin
      Grade.UnselectAll;
end;

procedure TNFE50.ValidarXML;
var
   Util: NFe_Util_2G_Interface;
   mQtdeErrosXML,
   i, mTipo: integer;
...
var
   ...
   ,xmlEPEC: WideString;
begin
      tMemoXML         := TMemo.Create(NFE50);
      tMemoXML.Visible := False;
      tMemoXML.Parent  := NFE50;
      Util             := CoUtil.Create;
...
procedure TNFE50.EnviarEmail(Tipo: String);
var
   mccList
...
procedure TNFE50.DANFE_RetratoBeforePrint(Sender: TObject);
begin
...
procedure TNFE50.GeraDuplicatas(Tipo: Integer; Subsequente: Boolean; Dias: Integer; Classificacao: String; PercentualCambio: Real);
Var
    mValor           : Currency;
    mDataVenc        : TDate;
    mVenc            : String;
...
procedure TNFE50.ItemGrade(Item: Integer; Tabela, Produto, NCM, Processo, Descricao: String);
begin
     with Dados do begin
          tErro.SQL.Clear;
...
procedure TNFE50.GradeCellClick(Column: TColumn);
begin
     Grade.Enabled := false;
     Grade.UnselectAll;
     Grade.SelectedRows.CurrentRowSelected := True;
...
procedure TNFE50.GradeDrawColumnCell(Sender: TObject; const Rect: TRect; DataCol: Integer; Column: TColumn; State: TGridDrawState);
var
    Icon: TBitmap;
begin
      With Dados do begin
...
procedure TNFE50.GradeKeyUp(Sender: TObject;var Key: Word; Shift: TShiftState);
begin
     PegaErros;
end;

procedure TNFE50.GradeEditChange(Sender: TObject);
begin
     PegaErros;
end;

procedure TNFE50.cProcessoChange(Sender: TObject);
begin
     FiltraPedidos;
end;

procedure TNFE50.FiltraPedidos;
begin
     With Dados do begin
          Grade.DisableScroll;
          Pedidos.SQL.Clear;
...
procedure TNFE50.cTipoClick(Sender: TObject);
begin
      FiltraPedidos;
end;

procedure TNFE50.tContainersCalcFields(DataSet: TDataSet);
begin
...
procedure TNFE50.PageControl1Change(Sender: TObject);
begin
     bEnviar.Enabled := PageControl1.ActivePageIndex = 0;
end;

procedure TNFE50.Button1Click(Sender: TObject);
Var
    mPesquisa : String;
begin
...
procedure TNFE50.Button2Click(Sender: TObject);
Var
    mSel:integer;
begin
     with Dados do begin
...
procedure TNFE50.bXMLClick(Sender: TObject);
Var
    mSel: Integer;
begin
...
procedure TNFE50.GradeErrosCellClick(Column: TColumn);
Var
   mPedido: Integer;
   mSQL   : WideString;
begin
...
Function TNFE50.MontaHistorico(Historico: integer):String;
begin
...
procedure TNFE50.RegistraPasso(pPasso: string);
var
    Salva: boolean;
begin
      with Dados do begin
...
procedure TNFE50.DuplicataDANFE(pParc:integer);
var
   Obj:TObject;
begin
...
procedure TNFE50.BloqueiaEmissao(Tipo: Boolean);
begin
...
// Envia a nota fiscal eletrônica para o web service.
function TNFE50.ConsultaContig: boolean;
var
   Util: NFe_Util_2G_Interface;
   cstat:integer;
begin
