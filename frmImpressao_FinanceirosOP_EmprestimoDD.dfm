object Impressao_FinanceirosOP_EmprestimoDD: TImpressao_FinanceirosOP_EmprestimoDD
  Left = 419
  Top = 216
  BorderStyle = bsDialog
  Caption = 'Impressao_FinanceirosOP_EmprestimoDD'
  ClientHeight = 300
  ClientWidth = 514
  Color = clBtnFace
  ...
  object RxLabel2: TRxLabel
    ...
    Width = 243
    Height = 18
    Caption = 'Empr'#233'stimos (Desconto de Duplicatas).'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWhite
    Font.Height = -15
    Font.Name = 'Calibri'
  ...
  object cSituacao: TRadioGroup
    ...
    Width = 285
    Height = 49
    Caption = 'Situa'#231#227'o das Duplicatas'
    Columns = 3
    ItemIndex = 2
    Items.Strings = (
      'Abertas'
  ...
  object cTotalEmprestimo: TCheckBox
    Left = 11
    Top = 218
    Width = 140
    Height = 17
    Alignment = taLeftJustify
    Caption = 'Totalizar por Empr'#233'stimo'
    Checked = True
    State = cbChecked
    TabOrder = 6
  end
  ...
  object cEmprestimo: TRxDBLookupCombo
    Left = 89
    Top = 180
    Width = 413
    Height = 21
    ...
    Font.Style = [fsBold]
    LookupField = 'Numero'
    LookupDisplay = 'Numero; Banco_Nome'
    LookupSource = Dados.dsEmprestimos
    ParentFont = False
    TabOrder = 5
  end
  object cConsol: TRadioGroup
    Left = 305
    Top = 56
    Width = 197
    Height = 49
    Caption = 'Consolida'#231#227'o'
    Columns = 2
    ItemIndex = 1
    Items.Strings = (
      'Individual'
  ...
  object rEmprestimosDD: TppReport
    AutoStop = False
    DataPipeline = ppEmprestimos
    PassSetting = psTwoPass
    PrinterSetup.BinName = 'Default'
    PrinterSetup.DocumentName = 'Empr'#233'stimos (Desconto de Duplicatas)'
    ...
    Template.FileName = 
      'C:\Sistemas\Faturamento\Codigo Fonte\Relatorios\EmprestimosDD.rt' +
      'm'
    ...
    Version = '19.04'
    mmColumnWidth = 0
    DataPipelineName = 'ppEmprestimos'
    ...
    object ppGroup1: TppGroup
      ...
      DataPipelineName = 'ppEmprestimos'
      NewFile = False
      object gCabEmprestimo: TppGroupHeaderBand
        Background.Brush.Style = bsClear
        Border.mmPadding = 0
        mmBottomOffset = 0
        mmHeight = 794
      ...
      object gRodaPEEmprestimo: TppGroupFooterBand
        Background.Brush.Style = bsClear
        Border.mmPadding = 0
        ...
        object ppLabel15: TppLabel
          ...
          AutoSize = False
          Border.mmPadding = 0
          Caption = 'TOTAL DO EMPR'#201'STIMO'
          Color = 12615680
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWhite
          Font.Name = 'Calibri'
  ...
  object tEmprestimos: TMSQuery
    Connection = Dados.Banco_Empresas
    SQL.Strings = (
      'USE Cybersoft_ALI_Matriz'
      
        'SELECT (SELECT Nome FROM Bancos WHERE Codigo = (SELECT Banco FRO' +
        'M Emprestimos EM WHERE EM.Tipo = '#39'DD'#39' AND EM.Numero = ED.Emprest' +
        'imo)) AS Banco,'
      
        '       (SELECT Garantia FROM Emprestimos EM WHERE EM.Tipo = '#39'DD'#39 +
        ' AND EM.Numero = ED.Emprestimo) AS Garantia,'
      
        '       (SELECT Tipo+CAST(Numero AS VARCHAR(5)) FROM Emprestimos ' +
        'EM WHERE EM.Tipo = '#39'DD'#39' AND EM.Numero = ED.Emprestimo) AS Numero' +
        ','
      
        '       (SELECT Data     FROM Emprestimos EM WHERE EM.Tipo = '#39'DD'#39 +
        ' AND EM.Numero = ED.Emprestimo) AS Data,'
      '       ED.Duplicata,'
      
        '       (SELECT Cliente FROM PagarReceber WHERE Numero = Financei' +
        'ro_Numero) AS Cliente_Codigo,'
      
        '       (SELECT Nome FROM Clientes WHERE Codigo = (SELECT Cliente' +
        ' FROM PagarReceber WHERE Numero = Financeiro_Numero)) AS Cliente' +
        '_Nome,'
      '       Vencimento,'
      '       Valor_Bruto,'
      '       Valor_Juros,'
      
        '       (SELECT Valor_IOF      FROM Emprestimos EM WHERE EM.Tipo ' +
        '= '#39'DD'#39' AND EM.Numero = ED.Emprestimo) / (SELECT COUNT(*) FROM Em' +
        'prestimosDuplicatas WHERE Emprestimo = ED.Emprestimo) AS Valor_I' +
        'OF,'
      
        '       (SELECT Valor_Diversos FROM Emprestimos EM WHERE EM.Tipo ' +
        '= '#39'DD'#39' AND EM.Numero = ED.Emprestimo) / (SELECT COUNT(*) FROM Em' +
        'prestimosDuplicatas WHERE Emprestimo = ED.Emprestimo) AS Valor_D' +
        'iversos,'
      '       (Valor_Bruto - Valor_Juros) AS Liquido_Creditado,'
      '       Valor_Juros + '
      
        '      (SELECT Valor_IOF      FROM Emprestimos EM WHERE EM.Tipo =' +
        ' '#39'DD'#39' AND EM.Numero = ED.Emprestimo) / (SELECT COUNT(*) FROM Emp' +
        'restimosDuplicatas WHERE Emprestimo = ED.Emprestimo) +'
      
        '      (SELECT Valor_Diversos FROM Emprestimos EM WHERE EM.Tipo =' +
        ' '#39'DD'#39' AND EM.Numero = ED.Emprestimo) / (SELECT COUNT(*) FROM Emp' +
        'restimosDuplicatas WHERE Emprestimo = ED.Emprestimo) AS Custo_Ef' +
        'etivo,'
      '       ED.Liquidar,'
      
        '       CASE WHEN ISNULL(Financeiro_Baixa, 0) = 0 THEN CAST(0 AS ' +
        'bit) ELSE CAST(1 AS bit) END AS Baixada,'
      '       Origem = '#39'MT '#39','
      '       Data_Liquidacao'
      'FROM   EmprestimosDuplicatas ED'
      
        'WHERE  (SELECT Tipo FROM Emprestimos EM WHERE EM.Tipo = '#39'DD'#39' AND' +
        ' EM.Numero = ED.Emprestimo) = '#39'DD'#39
      'ORDER  BY Numero, Duplicata'
      '')
    Left = 272
    Top = 170
  end
  ...
  object ppEmprestimos: TppDBPipeline
    DataSource = dstEmpestimos
    CloseDataSource = True
    UserName = 'Emprestimos'
    Left = 353
    Top = 221
  end
  object dstEmpestimos: TDataSource
    DataSet = tEmprestimos
    Left = 272
    Top = 221
  end
