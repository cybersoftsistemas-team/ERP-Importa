object Impressao_Financeiros_FluxoCaixa: TImpressao_Financeiros_FluxoCaixa
  Left = 321
  Top = 329
  BorderStyle = bsDialog
  Caption = 'Impressao_Financeiros_FluxoCaixa'
  ClientHeight = 203
  ClientWidth = 378
  Color = clBtnFace
  ...
  object tFluxo: TMSQuery
    ...
    SQL.Strings = (
      'SELECT Ano             = 2013,'
      '       Mes             = 1,'
      '       Nome_Mes        = '#39'Janeiro'#39','
      
        '       Total_Emprestimos = ISNULL((SELECT SUM(Valor) FROM Empres' +
        'timosParcelas EP WHERE MONTH(Vencimento) = 1 AND YEAR(Vencimento' +
        ') = 2013 AND (SELECT COUNT(*) FROM PagarReceberBaixas PB WHERE P' +
        'B.Numero = EP.Financeiro_Numero) = 0), 0)+'
      
        '                         ISNULL((SELECT SUM(Valor_ME * Taxa_Camb' +
        'ial) FROM EmprestimosFINIMP EF WHERE MONTH(Data) = 1 AND YEAR(Da' +
        'ta) = 2013 AND (SELECT COUNT(*) FROM PagarReceberBaixas PB WHERE' +
        ' PB.Numero = EF.Financeiro_Numero) = 0), 0),'
      
        '       Despesas_Fixas  = ISNULL((SELECT SUM(Valor_Total) FROM Pa' +
        'garReceber PR WHERE MONTH(Data_Vencimento) = 1 AND YEAR(Data_Ven' +
        'cimento) = 2013 AND (SELECT COUNT(*) FROM PagarReceberBaixas PB ' +
        'WHERE PB.Numero = PR.Numero) = 0 AND (SELECT Despesa_Fixa FROM C' +
        'lassificacaoFinanceira CF WHERE CF.Codigo = PR.Classificacao) = ' +
        '1), 0),'
      
        '       Despesas_NFixas = ISNULL((SELECT SUM(Valor_Total) FROM Pa' +
        'garReceber PR WHERE MONTH(Data_Vencimento) = 1 AND YEAR(Data_Ven' +
        'cimento) = 2013 AND ISNULL(Processo, '#39#39') = '#39#39' AND ISNULL(Emprest' +
        'imo, 0) = 0 AND (SELECT COUNT(*) FROM PagarReceberBaixas PB WHER' +
        'E PB.Numero = PR.Numero) = 0 AND (SELECT Despesa_Fixa FROM Class' +
        'ificacaoFinanceira CF WHERE CF.Codigo = PR.Classificacao) = 0), ' +
        '0),'
      
        '       Despesas_COMEX  = ISNULL((SELECT SUM(Valor_Total) FROM Pa' +
        'garReceber PR WHERE MONTH(Data_Vencimento) = 1 AND YEAR(Data_Ven' +
        'cimento) = 2013 AND ISNULL(Processo, '#39#39') <> '#39#39' ), 0),'
      '       Despesas_Outras = CAST(0 AS money),'
      
        '       Previsao_Nac    = ISNULL((SELECT SUM(ISNULL(Fator_SISCOME' +
        'XValor   , 0) * ISNULL(Taxa_FOB,0)) FROM ProcessosDocumentos WHE' +
        'RE YEAR(Data_RegistroDeclaracao) = 2013 AND MONTH(Data_RegistroD' +
        'eclaracao) = 1), 0),'
      '       Despesas_Total  = CAST(0 AS money),'
      
        '       Previsao_Fat    = ISNULL((SELECT SUM(ISNULL(Fator_Faturam' +
        'entoValor, 0) * ISNULL(Taxa_FOB,0)) FROM ProcessosDocumentos WHE' +
        'RE YEAR(Data_PrevFaturamento)    = 2013 AND MONTH(Data_PrevFatur' +
        'amento)    = 1), 0),'
      '       CAR             = 0,'
      '       Receitas_Outras = CAST(0 AS money),'
      '       Receitas_Total  = CAST(0 AS money),'
      '       Saldo_Mes       = CAST(0 AS money)'
      'INTO #TEMP'
      'UNION ALL'
      'SELECT Ano             = 2013,'
      '       Mes             = 2,'
      '       Nome_Mes        = '#39'Fevereiro'#39','
      
        '       Total_Emprestimos     = ISNULL((SELECT SUM(Valor) FROM Em' +
        'prestimosParcelas EP WHERE MONTH(Vencimento) = 2 AND YEAR(Vencim' +
        'ento) = 2013 AND (SELECT COUNT(*) FROM PagarReceberBaixas PB WHE' +
        'RE PB.Numero = EP.Financeiro_Numero) = 0), 0)+'
      
        '                         ISNULL((SELECT SUM(Valor_ME * Taxa_Camb' +
        'ial) FROM EmprestimosFINIMP EF WHERE MONTH(Data) = 2 AND YEAR(Da' +
        'ta) = 2013 AND (SELECT COUNT(*) FROM PagarReceberBaixas PB WHERE' +
        ' PB.Numero = EF.Financeiro_Numero) = 0), 0),'
      
        '       Despesas_Fixas  = ISNULL((SELECT SUM(Valor_Total) FROM Pa' +
        'garReceber PR WHERE MONTH(Data_Vencimento) = 2 AND YEAR(Data_Ven' +
        'cimento) = 2013 AND (SELECT COUNT(*) FROM PagarReceberBaixas PB ' +
        'WHERE PB.Numero = PR.Numero) = 0 AND (SELECT Despesa_Fixa FROM C' +
        'lassificacaoFinanceira CF WHERE CF.Codigo = PR.Classificacao) = ' +
        '1), 0),'
      ...
      '       Saldo_Mes       = CAST(0 AS money)'
        ...
        'UPDATE #TEMP SET Despesas_Total = Total_Emprestimos + Despesas_F' +
        'ixas + Despesas_NFixas + Despesas_COMEX + Despesas_Outras + Prev' +
        'isao_Nac,'
      '                 Receitas_Total = Previsao_Fat + Receitas_Outras'
      'UPDATE #TEMP SET Saldo_Mes = Receitas_Total - Despesas_Total'
      'SELECT * FROM #TEMP ORDER BY Ano, Mes'
      'DROP TABLE #TEMP')
    FetchRows = 1
    AfterScroll = tFluxoAfterScroll
    Left = 268
    Top = 112
  end
  ...
  object pFluxo: TppDBPipeline
    DataSource = dstFluxo
    CloseDataSource = True
    UserName = 'pFluxo'
    Left = 275
    Top = 8
    object pFluxoppField1: TppField
      Alignment = taRightJustify
      FieldAlias = 'Ano'
      FieldName = 'Ano'
      FieldLength = 0
    ...
    object pFluxoppField3: TppField
      FieldAlias = 'Nome_Mes'
      FieldName = 'Nome_Mes'
      FieldLength = 9
      DisplayWidth = 9
    ...
    object pFluxoppField4: TppField
      Alignment = taRightJustify
      FieldAlias = 'Total_Emprestimos'
      FieldName = 'Total_Emprestimos'
      FieldLength = 0
      DataType = dtDouble
      DisplayWidth = 10
      Position = 3
    end
    object pFluxoppField5: TppField
      FieldAlias = 'Despesas_Fixas'
    ...
    object pFluxoppField6: TppField
      FieldAlias = 'Despesas_NFixas'
      FieldName = 'Despesas_NFixas'
      FieldLength = 0
      DataType = dtCurrency
      DisplayWidth = 10
      Position = 5
    end
    object pFluxoppField7: TppField
      FieldAlias = 'Despesas_COMEX'
      FieldName = 'Despesas_COMEX'
      FieldLength = 0
      DataType = dtCurrency
      DisplayWidth = 10
      Position = 6
    end
    object pFluxoppField8: TppField
      FieldAlias = 'Despesas_Outras'
    ...
    object pFluxoppField9: TppField
      Alignment = taRightJustify
      FieldAlias = 'Previsao_Nac'
      FieldName = 'Previsao_Nac'
      FieldLength = 0
      DataType = dtDouble
      DisplayWidth = 10
      Position = 8
    end
    object pFluxoppField10: TppField
      FieldAlias = 'Despesas_Total'
    ...
    object pFluxoppField11: TppField
      Alignment = taRightJustify
      FieldAlias = 'Previsao_Fat'
      FieldName = 'Previsao_Fat'
      FieldLength = 0
      DataType = dtDouble
      DisplayWidth = 10
      Position = 10
    end
    object pFluxoppField12: TppField
      Alignment = taRightJustify
      FieldAlias = 'CAR'
      FieldName = 'CAR'
      FieldLength = 0
    ...
    object pFluxoppField13: TppField
      FieldAlias = 'Receitas_Outras'
      FieldName = 'Receitas_Outras'
      FieldLength = 0
      DataType = dtCurrency
      DisplayWidth = 10
      Position = 12
    end
    object pFluxoppField14: TppField
      FieldAlias = 'Receitas_Total'
      FieldName = 'Receitas_Total'
      FieldLength = 0
      DataType = dtCurrency
      DisplayWidth = 10
      Position = 13
    end
    object pFluxoppField15: TppField
