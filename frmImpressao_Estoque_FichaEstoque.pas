unit frmImpressao_Estoque_FichaEstoque;
...
uses
  ...
  ppReport, DB, DBAccess, MSAccess, RxLookup, Mask, Vcl.StdCtrls, Buttons, IniFiles,
   ComObj, RxCurrEdit, RxToolEdit, ppDB, ppDesignLayer, ppPrnabl, ppCache, ppComm, ppRelatv, ppProd, MemDS;

type
  TImpressao_Estoque_FichaEstoque = class(TForm)
    ...
    procedure pQtde_InicialOldOnCalc(Sender: TObject);
  private
...
var
  Impressao_Estoque_FichaEstoque: TImpressao_Estoque_FichaEstoque;
...
procedure TImpressao_Estoque_FichaEstoque.FormCreate(Sender: TObject);
begin
      If FileExists('fundo_barra.bmp') then Image1.Picture.LoadFromFile('fundo_barra.bmp');
end;

procedure TImpressao_Estoque_FichaEstoque.FormClose(Sender: TObject;var Action: TCloseAction);
Var
   aINI: TIniFile;
begin
      // Salvando as ultimas opções utilizadas no relatório como default.
      aIni := TIniFile.Create(ExtractFilePath(Application.ExeName)+'ImportaRelatorios.ini');
      aINI.WriteDate   ( 'IMPRESSAO_Ficha_Estoque','DataIni', cDataIni.Date);
      aINI.WriteDate   ( 'IMPRESSAO_Ficha_Estoque','DataFim', cDataFim.Date);
      aINI.WriteInteger( 'IMPRESSAO_Ficha_Estoque','Codigo' , cCodigo.AsInteger);
      aINI.WriteBool   ( 'IMPRESSAO_Ficha_Estoque','materia' , cMateria.Checked);
      If cProduto.KeyValue <> null then
         aINI.WriteInteger( 'IMPRESSAO_Ficha_Estoque','Produto', cProduto.KeyValue)
      else
         aINI.WriteInteger( 'IMPRESSAO_Ficha_Estoque','Produto', 0);
      aINI.Free;

      Impressao_Estoque_FichaEstoque.Release;
      Impressao_Estoque_FichaEstoque := nil;
end;

procedure TImpressao_Estoque_FichaEstoque.bSairClick(Sender: TObject);
begin
     Close;
end;

procedure TImpressao_Estoque_FichaEstoque.bImprimirClick(Sender: TObject);
begin
      With Dados do begin
           ...
           with FichaEstoque do begin  
                ...
                if cProduto.DisplayValue <> '' then begin
                   FichaEstoque.SQL.Add('and Codigo = :pCodigo');
                   if cMateria.Checked then begin
                     sql.add('or Codigo in(select Codigo_MateriaPrima from ProdutosMateriaPrima where Codigo_Produto = :pCodigo)');
                   end;
                   ParamByName('pCodigo').AsInteger := cCodigo.AsInteger;    
                end;
                sql.add('order by Codigo, Data, Registro');
                ParamByName('pDataIni').AsDate := cDataIni.Date;
                ParamByName('pDataFim').AsDate := cDataFim.Date;
                //SQL.SaveToFile('c:\temp\Ficha_Estoque.SQL');
                Open;
           end;
      End;
...
procedure TImpressao_Estoque_FichaEstoque.FormShow(Sender: TObject);
Var
   aINI: TIniFile;
begin
      Screen.Cursor := crSQLWait;
      ...
      aINI              := TIniFile.Create(ExtractFilePath(Application.ExeName)+'ImportaRelatorios.ini');
      cDataIni.Date     := aINI.ReadDate   ('IMPRESSAO_Ficha_Estoque','DataIni', Date);
      cDataFim.Date     := aINI.ReadDate   ('IMPRESSAO_Ficha_Estoque','DataFim', Date);
      cProduto.KeyValue := aINI.ReadString ('IMPRESSAO_Ficha_Estoque','Produto', '');
      cCodigo.Value     := aINI.ReadInteger('IMPRESSAO_Ficha_Estoque','Codigo' , 0);
      cMateria.Checked  := aINI.ReadBool   ('IMPRESSAO_Ficha_Estoque','Materia', false);
      aINI.Free;
...
procedure TImpressao_Estoque_FichaEstoque.pQtde_InicialOldOnCalc(Sender: TObject);
begin
     SaldoInicial;
end;

procedure TImpressao_Estoque_FichaEstoque.bLimparFormaClick(Sender: TObject);
begin
      cProduto.ClearValue;
end;

procedure TImpressao_Estoque_FichaEstoque.cCodigoExit(Sender: TObject);
begin
...
procedure TImpressao_Estoque_FichaEstoque.cProdutoChange(Sender: TObject);
begin
     cCodigo.Clear;
...
procedure TImpressao_Estoque_FichaEstoque.ExportaEXCEL;
var
   mPlanilha: Variant;
...
begin
       With dmFiscal, Dados do begin
            ...
            Janela_Processamento.Progresso.Max      := FichaEstoque.RecordCount;
            ...
            mPlanilha.Cells[2,01] := 'Ficha de Estoque - Período de '+cDataIni.Text + ' a '+cDataFim.Text;
            ...
            While (not FichaEstoque.Eof) and (not Funcoes.Cancelado) do  Begin
                  // Quebra do item.
                  if FichaEstoqueCodigo.AsInteger <> mCodigo then begin
                     If mCodigo > 0 then Inc(mLinha);
                     mCodigo := FichaEstoqueCodigo.AsInteger;

                     // Descrição da mercadoria.
                     mPlanilha.Cells[mLinha,01] := 'PRODUTO: '+ FichaEstoque.FieldByName('Codigo').AsString+' - '+FichaEstoque.FieldByName('Descricao').AsString;
                  ...
                  mPlanilha.Cells[mLinha,01] := FichaEstoque.FieldByName('Data').AsDateTime;
                  mPlanilha.Cells[mLinha,02] := FichaEstoque.FieldByName('Nota').AsInteger;
                  mPlanilha.Cells[mLinha,03] := FichaEstoque.FieldByName('CFOP').AsString;
                  mPlanilha.Cells[mLinha,04] := FichaEstoque.FieldByName('Historico').AsString;
                  mPlanilha.Cells[mLinha,05] := FichaEstoque.FieldByName('Destinatario_Nome').AsString;
                  mPlanilha.Cells[mLinha,06] := FichaEstoque.FieldByName('Qtde_Entrada').AsFloat;
                  mPlanilha.Cells[mLinha,07] := FichaEstoque.FieldByName('Unitario_Entrada').AsCurrency;
                  mPlanilha.Cells[mLinha,08] := FichaEstoque.FieldByName('Total_Entrada').AsCurrency;
                  mPlanilha.Cells[mLinha,09] := FichaEstoque.FieldByName('Qtde_Saida').AsFloat;
                  mPlanilha.Cells[mLinha,10] := FichaEstoque.FieldByName('Unitario_Saida').AsCurrency;
                  mPlanilha.Cells[mLinha,11] := FichaEstoque.FieldByName('Total_Saida').AsCurrency;
                  mPlanilha.Cells[mLinha,12] := FichaEstoque.FieldByName('Qtde_Saldo').AsFloat;
                  mPlanilha.Cells[mLinha,13] := FichaEstoque.FieldByName('Unitario_Saldo').AsCurrency;
                  mPlanilha.Cells[mLinha,14] := FichaEstoque.FieldByName('Total_Saldo').AsCurrency;
...
procedure TImpressao_Estoque_FichaEstoque.SaldoInicial;
begin
      ...
      with Dados do begin
           ...
           tSaldo.SQL.Add('where  Registro = (select max(Registro) from FichaEstoque where Codigo = :pCodigo and Data < :pData)');
           ...
           //tSaldo.SQL.SaveToFile('c:\temp\Ficha_Estoque_Saldo_Inicial.sql');
           tSaldo.Open;
