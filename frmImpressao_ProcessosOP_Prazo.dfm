object Impressao_ProcessosOP_Prazo: TImpressao_ProcessosOP_Prazo
  Left = 368
  Top = 191
  BorderStyle = bsDialog
  Caption = 'Impressao_ProcessosOP_Prazo'
  ClientHeight = 345
  ClientWidth = 559
  Color = clBtnFace
  ...
  object cFechamento: TRadioGroup
    Left = 8
    Top = 112
    Width = 543
    Height = 47
    Caption = 'Situa'#231#227'o do Fechamento'
    Columns = 3
    ItemIndex = 0
    Items.Strings = (
      'Definitivos'
  ...
  object tProcessos: TMSQuery
    ...
    SQL.Strings = (
      ...
      '       Processo_Abertura AS Data_Abertura,'
      '       Data_RegistroDeclaracao AS Data_Registro,'
      '       Data_Encerramento,'
      '       Processo_Fechamento AS Data_Fechamento,'
      
        '       ISNULL(DATEDIFF(Day, Data_RegistroDeclaracao, Processo_Ab' +
        'ertura),0) AS Dias_Abertura,'
      
        '       ISNULL(DATEDIFF(Day, Data_RegistroDeclaracao, Data_Encerr' +
        'amento ),0) AS Dias_Encerramento,'
      '       CASE WHEN Processo_Fechamento IS NOT NULL THEN '
      
        '            ISNULL(DATEDIFF(Day, Data_Encerramento, Processo_Fec' +
        'hamento),0)'
      '       ELSE'
      
        '            ISNULL(DATEDIFF(Day, Data_Encerramento, GETDATE()),0' +
        ')'
      '       END AS Dias_Fechamento,'
      
        '       (SELECT CAST(Observacao AS VARCHAR(120)) FROM ProcessosFe' +
        'chamento WHERE(ProcessosFechamento.Processo = ProcessosDocumento' +
        's.Processo)) AS Observacao'
      'FROM   ProcessosDocumentos '
      'ORDER  BY Processo')
    Left = 256
    Top = 8
  end
  object rProcessosPrazos: TppReport
    AutoStop = False
    DataPipeline = pProcessos
    PrinterSetup.BinName = 'Default'
    PrinterSetup.DocumentName = 'Prazos dos Processos'
    ...
    Template.FileName = 
      'C:\Sistemas\Faturamento\Codigo Fonte\Relatorios\ProcessosPrazos.' +
      'rtm'
