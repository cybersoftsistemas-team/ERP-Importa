unit frmImpressao_FaturamentoOP_Estoque;

interface

uses
  Winapi.Windows, Winapi.Messages, System.SysUtils, System.Variants, System.Classes, Vcl.Graphics, Vcl.Controls, Vcl.Forms, Vcl.Dialogs, ppVar, DB, MemDS, DBAccess, MSAccess, ppDB, ppDBPipe, ppParameter, ppStrtch, ppMemo, ppBands, ppClass, ppCtrls, ppPrnabl,
  ppCache, ppComm, ppRelatv, ppProd, ppReport, DBCtrls, Vcl.StdCtrls, Vcl.ExtCtrls, RxLookup, RXCtrls, Mask, Funcoes, Buttons, INIFiles, ComObj, MaskUtils, RxCurrEdit, RxToolEdit;

type
  TImpressao_FaturamentoOP_Estoque = class(TForm)
    XPPanel1: TXPPanel;
    RxLabel1: TRxLabel;
    RxLabel2: TRxLabel;
    iJatoTinta: TImage;
    ...
    cDestinatario: TRxDBLookupCombo;
...
var
  Impressao_FaturamentoOP_Estoque: TImpressao_FaturamentoOP_Estoque;
...
procedure TImpressao_FaturamentoOP_Estoque.FormCreate(Sender: TObject);
begin
      If FileExists('fundo_barra.bmp') then XPPanel1.BgImage.LoadFromFile('fundo_barra.bmp');
end;

procedure TImpressao_FaturamentoOP_Estoque.bSairClick(Sender: TObject);
begin
     Close;
end;

procedure TImpressao_FaturamentoOP_Estoque.FormClose(Sender: TObject;var Action: TCloseAction);
var
   aINI: TIniFile;
begin
      ...
      If Trim(cDestinatario.Text) <> '' then
         aINI.WriteString('IMPRESSAO_FATURAMENTO_ESTOQUE', 'Destinatario'  , tDestinatarioCNPJ_CPF.AsString)
      else
         aINI.WriteString('IMPRESSAO_FATURAMENTO_ESTOQUE', 'Destinatario'  , '');
      aINI.Free;
      ...
      Impressao_FaturamentoOP_Estoque.Release;
      Impressao_FaturamentoOP_Estoque := nil;
end;

procedure TImpressao_FaturamentoOP_Estoque.FormShow(Sender: TObject);
var
   aINI: TIniFile;
begin
      ...
      With Dados do begin
           ...
           cDestinatario.KeyValue := aINI.ReadString ('IMPRESSAO_FATURAMENTO_ESTOQUE', 'Destinatario', '');
...
procedure TImpressao_FaturamentoOP_Estoque.bImprimirClick(Sender: TObject);
begin
      ...
      tItens.SQL.Add('       NI.Descricao_Mercadoria,');
      ...
      tItens.SQL.Add('WHERE  (NI.Movimenta_Estoque = 1) AND (NI.Cancelada <> 1) AND (NF.Numero = NI.Nota) AND (NF.Data_Emissao = NI.Data)');
      ...
      End;
      If Trim(cDestinatario.DisplayValue) <> '' then begin
         tItens.SQL.Add('       AND (NF.Destinatario_CNPJ_CPF = :pCNPJ)');
         tItens.ParamByName('pCNPJ').AsString := ApenasNumeros(tDestinatarioCNPJ_CPF.AsString);
      End;
      ...
      tItens.SQL.Add('       Descricao_Mercadoria = null,');
      tItens.SQL.Add('       PT.Data_Transferencia,');
      tItens.SQL.Add('       null,');
      tItens.SQL.Add('       null,');
      tItens.SQL.Add('       Nota_Compl        = null,');
      tItens.SQL.Add('       Entrada_Compra    = Quantidade,');
      ...
      tItens.SQL.Add('       Total_Item        = 0,');
      tItens.SQL.Add('       Valor_Unitario    = 0,');
      tItens.SQL.Add('       Destinatario_Nome = CASE WHEN Motivo = ''T'' THEN ''*** TRANSFERÊNCIA DE ESTOQUE (ENTRADA) ***'' ELSE ''*** SALDO DE ABERTURA DE ESTOQUE ***'' END');
      ...
      tItens.SQL.Add('       Entrada_Devol     = 0,');
      tItens.SQL.Add('       Saida_Devol       = 0,');
      tItens.SQL.Add('       Saida_Venda       = Quantidade,');
      tItens.SQL.Add('       Saida_Terceiros   = 0,');
      tItens.SQL.Add('       Saida_Outras      = 0,');
      tItens.SQL.Add('       Total_Item        = 0,');
      tItens.SQL.Add('       Valor_Unitario    = 0,');
      ...
      If Trim(cTipoNota.DisplayValue) = '' then begin
         tItens.SQL.Add('UNION ALL');
         tItens.SQL.Add('SELECT Tipo = CASE WHEN NT.Provisoria = 0 THEN ''T'' ELSE ''V'' END,');
         tItens.SQL.Add('       Sai_Entra = 0,');
         tItens.SQL.Add('       NTI.Codigo_Mercadoria,');
         tItens.SQL.Add('       null,');
         tItens.SQL.Add('       NTI.Processo,');
         tItens.SQL.Add('       NTI.Descricao_Mercadoria,');
         tItens.SQL.Add('       NTI.Data_Entrada,');
         tItens.SQL.Add('       NTI.Nota,');
         tItens.SQL.Add('       CFOP = NTI.Natureza_Codigo,');
         tItens.SQL.Add('       Nota_Compl        = null,');
         tItens.SQL.Add('       Entrada_Compra    = CASE WHEN ((SELECT Finalidade_Mercadoria FROM ReferenciasFiscais WHERE(Codigo = NT.Referencia_Fiscal)) IN(0, 3, 5, 6, 9)) THEN NTI.Quantidade ELSE 0 END,');
         tItens.SQL.Add('       Entrada_Terceiros = CASE WHEN ((SELECT Finalidade_Mercadoria FROM ReferenciasFiscais WHERE(Codigo = NT.Referencia_Fiscal)) = 4) THEN NTI.Quantidade ELSE 0 END,');
         tItens.SQL.Add('       Entrada_Devol     = CASE WHEN ((SELECT Finalidade_Mercadoria FROM ReferenciasFiscais WHERE(Codigo = NT.Referencia_Fiscal)) = 2) THEN NTI.Quantidade ELSE 0 END,');
         tItens.SQL.Add('       Saida_Devol       = 0,');
         tItens.SQL.Add('       Saida_Venda       = 0,');
         tItens.SQL.Add('       Saida_Terceiros   = 0,');
         tItens.SQL.Add('       Saida_Outras      = 0,');
         ...
         tItens.SQL.Add('WHERE  (Movimenta_Estoque = 1) AND (NT.Nota = NTI.Nota) AND (NT.Data_Emissao = NTI.Data_Emissao) AND (NTI.Fornecedor = NT.Fornecedor)');

         If Trim(Removecaracter('/', '', cData.Text)) <> '' then begin
            tItens.SQL.Add('       AND (NT.Data_Entrada <= :pData)');
            tItens.ParamByName('pData').AsDate := cData.Date;
         End;
         If cCodigoProduto.Value > 0 then begin
         ...
         End;
         If Trim(cCFOP.DisplayValue) <> '' then begin
            tItens.SQL.Add('       AND (NT.Natureza_Codigo = :pCFOP)');
            tItens.ParamByName('pCFOP').AsString := tNatureza.FieldByName('Codigo').AsString;
         End;
         If cANP.Checked = true then begin
         ...
         End;
         If Trim(cDestinatario.DisplayValue) <> '' then begin
            tItens.SQL.Add('       AND (SELECT CNPJ FROM Fornecedores WHERE(Codigo = NT.Fornecedor)) = :pCNPJ');
            tItens.ParamByName('pCNPJ').AsString := ApenasNumeros(tDestinatarioCNPJ_CPF.AsString);
         End;
      End;
      ...
      If cPedidos.Checked = true then begin
         ...
         tItens.SQL.Add('       Valor_Unitario    = Valor_Unitario,');
         tItens.SQL.Add('       Destinatario_Nome');
         tItens.SQL.Add('FROM   PedidosItens NI, Pedidos NF');
      ...
      tItens.SQL.Add('ORDER BY Codigo_Mercadoria, Data, Nota');
      //tItens.SQL.SavetoFile('c:\temp\Posicao_Estoque.SQL');
      titens.Open;
...
procedure TImpressao_FaturamentoOP_Estoque.lSaldoTerceirosCalc(Sender: TObject; var Value: Variant);
begin
      Value := Value + tItens.FieldByName('Saida_Terceiros').AsFloat - tItens.FieldByName('Entrada_Terceiros').AsFloat;
end;

procedure TImpressao_FaturamentoOP_Estoque.gRodaPeProdutoAfterPrint(Sender: TObject);
begin
...
procedure TImpressao_FaturamentoOP_Estoque.lSaldoEmpresaCalc(Sender: TObject; var Value: Variant);
begin
...
procedure TImpressao_FaturamentoOP_Estoque.ppDetailBand1BeforePrint(Sender: TObject);
begin
...
procedure TImpressao_FaturamentoOP_Estoque.gRodaPeProdutoBeforePrint(Sender: TObject);
begin
...
procedure TImpressao_FaturamentoOP_Estoque.bLimparClick(Sender: TObject);
begin
      cData.Date := Date;
...
procedure TImpressao_FaturamentoOP_Estoque.cCodigoProdutoChange(Sender: TObject);
begin
...
procedure TImpressao_FaturamentoOP_Estoque.cProdutoChange(Sender: TObject);
begin
     cCodigoProduto.Value := Dados.produtosCodigo.Value;
end;

// Envia informações para o excel.
procedure TImpressao_FaturamentoOP_Estoque.ExportaEXCEL;
var
   mPlanilha   : Variant;
   mLinha      : Integer;
   mTotal01    : Real;
...
begin
       ...
       mPlanilha.Cells[mLinha, 1] := tItens.FieldByName('Descricao_Mercadoria').AsString;
       ...
       While (not tItens.Eof) and (not Funcoes.Cancelado) do  Begin
             ...
             mSaldoTerc := mSaldoTerc + tItens.FieldByName('Entrada_Terceiros').AsFloat - tItens.FieldByName('Saida_Terceiros').AsFloat;
             mSaldoEmp  := mSaldoEmp  + tItens.FieldByName('Entrada_Compra').AsFloat + tItens.FieldByName('Entrada_Devol').AsFloat + tItens.FieldByName('Entrada_Terceiros').AsFloat -
                                        tItens.FieldByName('Saida_Venda').AsFloat - tItens.FieldByName('Saida_Terceiros').AsFloat - tItens.FieldByName('Saida_Outras').AsFloat;
             ...
             If (mProduto <> tItens.FieldByName('Codigo_Mercadoria').AsInteger) or (tItens.Eof) then begin
                ...
                mPlanilha.Cells[mLinha,16] := mTotalProd06;
                mPlanilha.Cells[mLinha,17] := mTotalProd06;
...
procedure TImpressao_FaturamentoOP_Estoque.cProcessoChange(Sender: TObject);
begin
      With Dados do begin
           Produtos.SQL.Clear;
...
procedure TImpressao_FaturamentoOP_Estoque.bPesquisaCEPClick(Sender: TObject);
begin
      Screen.Cursor := crSQLWait;
