object Impressao_ProcessosOP_ControleInvoice: TImpressao_ProcessosOP_ControleInvoice
  Left = 357
  Top = 229
  BorderStyle = bsDialog
  Caption = 'Impressao_ProcessosOP_ControleInvoice'
  ClientHeight = 286
  ClientWidth = 559
  Color = clBtnFace
  ...
  object RxLabel2: TRxLabel
    ...
    Width = 216
    Height = 18
    Caption = 'Impress'#227'o do Controle de Invoices.'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWhite
    Font.Height = -15
    Font.Name = 'Calibri'
  ...
  object cVencimento: TRadioGroup
    Left = 316
    Top = 150
    Width = 236
    Height = 95
    ...
    ItemIndex = 0
    Items.Strings = (
      'Dentro do Per'#237'odo'
      'Dentro do Per'#237'odo + Sem Vencimentos'
      'Apenas sem Vencimento')
    TabOrder = 13
  end
  object cDesativados: TCheckBox
    Left = 8
    Top = 184
    Width = 193
    Height = 17
    Caption = 'N'#227'o Imprimnir procesos desativados'
    TabOrder = 14
  end
  object cSaldoZero: TCheckBox
    Left = 8
    Top = 163
    Width = 242
    Height = 17
    Caption = 'N'#227'o Imprimnir procesos com saldo igual a zero'
    TabOrder = 15
  end
  ...
  object pInvoice: TppDBPipeline
    ...
    object pInvoiceppField16: TppField
      FieldAlias = 'Cambios_PagosME'
      FieldName = 'Cambios_PagosME'
      FieldLength = 0
      DataType = dtCurrency
      DisplayWidth = 10
      Position = 15
    ...
    object pInvoiceppField17: TppField
      FieldAlias = 'Saldo_CambiosPagos'
      FieldName = 'Saldo_CambiosPagos'
      FieldLength = 0
      DataType = dtCurrency
      DisplayWidth = 10
      Position = 16
  ...
  object tInvoice: TMSQuery
    ...
    SQL.Strings = (
      'SELECT IV.Processo AS Ordem,'
      '       1 AS Item,'
      '       IV.Processo,'
      
        '       Cliente    = (SELECT Nome FROM Clientes     CL WHERE(CL.C' +
        'odigo = (SELECT Cliente FROM ProcessosDocumentos PD WHERE(PD.Pro' +
        'cesso = IV.Processo)) )),'
      
        '       Exportador = (SELECT Nome FROM Fornecedores FR WHERE(FR.C' +
        'odigo = IV.Exportador)),'
      '       Numero = LTRIM(RTRIM(IV.Numero)),'
      
        '       Condicao_Cambial = (SELECT Descricao FROM CondicaoCambial' +
        ' WHERE(Codigo = Condicao_Cambio)),'
      '       Total_FaturaME,'
      ''
      
        '       Valor_Antecipado = Total_FaturaME * ((100 - (SELECT ISNUL' +
        'L(Percentual1,0)+ISNULL(Percentual2,0)+ISNULL(Percentual3,0) FRO' +
        'M CondicaoCambial WHERE(Codigo = Condicao_Cambio)))/100),'
      
        '       Valor1           = Total_FaturaME * ((SELECT ISNULL(Perce' +
        'ntual1,0) FROM CondicaoCambial WHERE(Codigo = Condicao_Cambio))/' +
        '100),'
      
        '       Vencimento1      = CONVERT(VARCHAR(12), (SELECT Data_BL F' +
        'ROM ProcessosDocumentos PD WHERE(PD.Processo = IV.Processo)) + (' +
        'SELECT ISNULL(Vencimento1,0) FROM CondicaoCambial CB WHERE(CB.Co' +
        'digo = IV.Condicao_Cambio)), 103),'
      
        '       Valor2           = Total_FaturaME * ((SELECT ISNULL(Perce' +
        'ntual2,0) FROM CondicaoCambial WHERE(Codigo = Condicao_Cambio))/' +
        '100),'
      
        '       Vencimento2      = CONVERT(VARCHAR(12), (SELECT Data_BL F' +
        'ROM ProcessosDocumentos PD WHERE(PD.Processo = IV.Processo)) + (' +
        'SELECT ISNULL(Vencimento2,0) FROM CondicaoCambial CB WHERE(CB.Co' +
        'digo = IV.Condicao_Cambio)), 103),'
      
        '       Valor3           = Total_FaturaME * ((SELECT ISNULL(Perce' +
        'ntual3,0) FROM CondicaoCambial WHERE(Codigo = Condicao_Cambio))/' +
        '100),'
      
        '       Vencimento3      = CONVERT(VARCHAR(12), (SELECT Data_BL F' +
        'ROM ProcessosDocumentos PD WHERE(PD.Processo = IV.Processo)) + (' +
        'SELECT ISNULL(Vencimento3,0) FROM CondicaoCambial CB WHERE(CB.Co' +
        'digo = IV.Condicao_Cambio)), 103),'
      '       '
      
        '       Cambios_PagosME = ISNULL((SELECT SUM(Total_ME) FROM Contr' +
        'atoCambio CC WHERE(CC.Processo = IV.Processo)),0),'
      
        '       Saldo_CambiosPagos = Total_FaturaME - (SELECT SUM(Total_M' +
        'E) FROM ContratoCambio CC WHERE(CC.Processo = IV.Processo))'
      'FROM   Invoice IV'
      'WHERE  Processo = '#39'ABRTO 0031/12'#39
      ''
      '')
    Active = True
    Left = 258
  ...
  object rControleInvoice: TppReport
    AutoStop = False
    DataPipeline = pInvoice
    PrinterSetup.BinName = 'Default'
    PrinterSetup.DocumentName = 'Controle de Invoice'
    ...
    Template.FileName = 
      'C:\Sistemas\Faturamento\Codigo Fonte\Relatorios\ControleInvoice.' +
      'rtm'
