object Impressao_ProcessosOP_LimiteRADAR: TImpressao_ProcessosOP_LimiteRADAR
  Left = 0
  Top = 0
  BorderStyle = bsDialog
  Caption = 'Impressao_ProcessosOP_LimiteRADAR'
  ClientHeight = 192
  ClientWidth = 528
  Color = clBtnFace
  ...
  object RxLabel1: TRxLabel
    ...
    Width = 173
    Height = 21
    Caption = 'IMPRESS'#195'O - PROCESSO'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWhite
    Font.Height = -17
    Font.Name = 'Calibri'
  ...
  object RxLabel2: TRxLabel
    ...
    Width = 179
    Height = 18
    Caption = 'Controle de saldo do RADAR.'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWhite
    Font.Height = -15
    Font.Name = 'Calibri'
  ...
  object rRADAR: TppReport
    AutoStop = False
    DataPipeline = pRADAR
    NoDataBehaviors = [ndBlankReport]
    PassSetting = psTwoPass
    PrinterSetup.BinName = 'Default'
    PrinterSetup.DocumentName = 'Controle Limite de RADAR'
    ...
    object ppHeaderBand1: TppHeaderBand
      ...
      object lTitulo: TppLabel
        ...
        Border.Weight = 1.000000000000000000
        Border.mmPadding = 0
        Caption = 'CONTROLE DE LIMITE DE RADAR'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Calibri'
        Font.Size = 12
    ...
    object ppSummaryBand1: TppSummaryBand
      ...
      object ppLabel16: TppLabel
        ...
        Border.Visible = True
        Border.mmPadding = 101600
        Caption = ' TOTAL CIF(ME)'
        Color = 10092543
        Ellipsis = True
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
  ...
  object pRADAR: TppDBPipeline
    DataSource = dstRadar
    UserName = 'pRADAR'
    Left = 364
    Top = 61
  end
  object dstRadar: TDataSource
    DataSet = tRadar
    Left = 245
    Top = 111
  end
  object tRadar: TMSQuery
    Connection = Dados.Banco_Empresas
    SQL.Strings = (
      'use Cybersoft_RB'
      'select pd.Processo'
      
        '      ,Data_Abetura = convert(varchar, cast(pd.Processo_Abertura' +
        ' as date), 103)'
      
        '      ,Periodo = convert(varchar, Datediff(month, pd.Data_Regist' +
        'roDeclaracao, getdate()), 103)'
      
        '      ,Cliente = (select Nome from Clientes where Codigo = pd.Cl' +
        'iente)'
      '      ,DI = Numero_Declaracao'
      
        '      ,Exportador = (select Nome from Fornecedores where Codigo ' +
        '= pd.Fornecedor)'
      
        '      ,Data_Registro = convert(varchar, cast(pd.Data_RegistroDec' +
        'laracao as date), 103)'
      
        '      ,Data_Desemb = convert(varchar, cast(pd.Data_DesembaracoDe' +
        'claracao as date), 103)'
      
        '      ,Data_LibLimite = convert(varchar, cast(getdate() as date)' +
        ', 103)'
      '      ,Situacao_Limite = '#39'LIBERADO'#39
      '      ,Origem'
      
        '      ,Via_Transporte = (select Descricao from Cybersoft_Cadastr' +
        'os.dbo.ViaTransporte where Codigo = pd.Via_Transporte)'
      '      ,Canal_SISCOMEX = case pd.Canal_SISCOMEX'
      '                             when '#39'VD'#39' then '#39'VERDE'#39
      '                             when '#39'AM'#39' then '#39'AMARELO'#39
      '                             when '#39'VM'#39' then '#39'VERMELHO'#39
      '                             when '#39'CZ'#39' then '#39'CINZA'#39
      '                         end'
      
        #9'  ,Limite = (select Valor_RADAR from Clientes where Codigo = pd' +
        '.Cliente)'
      #9'  ,FOB_ME'
      #9'  ,Taxa_FOB = round(Taxa_FOB, 4)'
      #9'  ,FOB'
      
        #9'  ,Data_Limite = convert(varchar, cast(DATEADD(day, 180, Data_R' +
        'egistroDeclaracao) as date), 103)'
      #9'  ,Situacao = '#39'LIBERADO'#39
      'from   ProcessosDocumentos pd'
      'where Cliente = 5721'
      '--and Datediff(month, Data_RegistroDeclaracao, getdate()) <= 6'
      'order by Cliente'
      '')
    Left = 245
    Top = 61
