object Impressao_Faturamento_VendasMedia: TImpressao_Faturamento_VendasMedia
  Left = 0
  Top = 0
  BorderStyle = bsDialog
  Caption = 'Impressao_Faturamento_VendasMedia'
  ClientHeight = 306
  ClientWidth = 657
  Color = clBtnFace
  ...
  object RxLabel2: TRxLabel
    ...
    Width = 189
    Height = 18
    Caption = 'M'#233'dia de Vendas de Produtos.'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWhite
    Font.Height = -15
    Font.Name = 'Calibri'
  ...
  object cProduto: TRxDBLookupCombo
    ...
    Font.Style = [fsBold]
    LookupField = 'Codigo'
    LookupDisplay = 'Codigo;Codigo_Fabricante;Descricao'
  ...
  object cMedia: TCheckBox
    Left = 13
    Top = 218
    Width = 252
    Height = 17
    Caption = 'Apenas Produtos com Estoque Abaixo da M'#233'dia'
    Checked = True
    State = cbChecked
    TabOrder = 13
  end
  ...
  object tItens: TMSQuery
    ...
    SQL.Strings = (
      ...
      'set @DataIni = '#39'01/01/2023'#39
      'set @DataFim = '#39'02/28/2023'#39
      ''
      
        'select Linha = (select Descricao from ProdutosLinhas pl where pl' +
        '.Codigo = prd.Linha)'
      '      ,prd.Codigo     '
      
        '      ,Descricao = substring(Descricao, 1, charindex('#39'<{'#39', Descr' +
        'icao)-1)'
      '      ,Vendas = (select sum(Quantidade) '
      #9'         from  PedidosRepresentantesItens pri '
      #9'         where Pri.Data between @DataIni and @DataFim '
      #9#9' and   pri.Codigo_Mercadoria = prd.Codigo '
      
        #9#9' and  (select Cancelado from PedidosRepresentantes pr where pr' +
        '.pedido = pri.pedido) <> 1'
      
        #9#9' and  (select Local from PedidosRepresentantes pr where pr.ped' +
        'ido = pri.pedido) > 3 )'
      '      ,Media = (select sum(Quantidade) '
      #9'        from  PedidosRepresentantesItens pri '
      '                where Pri.Data between @DataIni and @DataFim '
      #9'        and   pri.Codigo_Mercadoria = prd.Codigo '
      
        #9'        and  (select Cancelado from PedidosRepresentantes pr wh' +
        'ere pr.pedido = pri.pedido) <> 1'
      
        #9'        and  (select Local from PedidosRepresentantes pr where ' +
        'pr.pedido = pri.pedido) > 3'
      #9'       ) / (DATEDIFF(month, @dataIni, @DataFim)+1)'
      
        '      ,Estoque = cast((select isnull(sum(Quantidade), 0)        ' +
        ' from NotasItens ni                  where ni.Codigo_Mercadoria ' +
        ' = Prd.Codigo and ni.Saida_Entrada = 0 and ni.Movimenta_EstoqueR' +
        'ep = 1 and ni.Cancelada <> 1 and isnull(ni.Nfe_Denegada, 0) = 0)' +
        ' + '
      
        '                      (select isnull(sum(Quantidade), 0)        ' +
        ' from NotasTerceirosItens nti        where nti.Codigo_Mercadoria' +
        ' = prd.Codigo and nti.Movimenta_Estoque = 1) + '
      
        '                      (select isnull(sum(Quantidade_Entrada), 0)' +
        ' from ProdutosTransferencia pt       where pt.Produto_Entrada = ' +
        'prd.Codigo) -'
      
        '                      (select isnull(sum(Quantidade), 0)        ' +
        ' from NotasItens  ni                 where ni.Codigo_Mercadoria ' +
        ' = prd.Codigo and ni.Saida_Entrada = 1 and ni.Movimenta_EstoqueR' +
        'ep = 1 and ni.Cancelada <> 1 and isnull(ni.Nfe_Denegada, 0) = 0)' +
        ' -'
      
        '                      (select isnull(sum(Quantidade), 0)        ' +
        ' from PedidosItens pi                where pi.Codigo_Mercadoria ' +
        ' = prd.Codigo and pi.Saida_Entrada = 1 and pi.Movimenta_EstoqueR' +
        'ep = 1 and pi.Pedido <> 0 ) -'
      
        '                      (select isnull(sum(Quantidade), 0)        ' +
        ' from ProdutosTransferencia pt       where pt.Produto_Saida = pr' +
        'd.Codigo) - '
      
        '                      (select isnull(sum(Quantidade), 0)        ' +
        ' from PedidosRepresentantesItens pri where pri.Codigo_Mercadoria' +
        ' = prd.Codigo)'
      '                 as decimal (14,8))'
      'from Produtos prd'
      'where Desativado <> 1'
      
        'and'#9'  (select sum(Quantidade) from PedidosRepresentantesItens pr' +
        'i where Pri.Data between @DataIni and @DataFim and pri.Codigo_Me' +
        'rcadoria = prd.Codigo and (select Cancelado from PedidosRepresen' +
        'tantes pr where pr.pedido = pri.pedido) <> 1 and (select Local f' +
        'rom PedidosRepresentantes pr where pr.pedido = pri.pedido) > 3 )' +
        ' > 0'
      'group by Linha, Codigo, Descricao'
  ...
  object rVendas: TppReport
    AutoStop = False
    DataPipeline = pItens
    NoDataBehaviors = [ndBlankReport]
    PrinterSetup.BinName = 'Default'
    PrinterSetup.DocumentName = 'Faturamento - Vendas'
    ...
    Template.FileName = 'C:\Sistemas\Faturamento\Codigo Fonte\Relatorios\rVendas.rtm'
    ...
    XLSSettings.Subject = 'M'#233'dia de Vendas'
    XLSSettings.Title = 'M'#233'dia de Vendas de Produtos'
    XLSSettings.WorksheetName = 'VENDAS - MEDIA'
    Left = 549
    Top = 8
    Version = '19.04'
    mmColumnWidth = 0
