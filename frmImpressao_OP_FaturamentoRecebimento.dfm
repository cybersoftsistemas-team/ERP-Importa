object Impressao_OP_FaturamentoRecebimento: TImpressao_OP_FaturamentoRecebimento
  Left = 0
  Top = 0
  BorderStyle = bsDialog
  Caption = 'Impressao_OP_FaturamentoRecebimento'
  ClientHeight = 208
  ClientWidth = 537
  Color = clBtnFace
  ...
  object RxLabel2: TRxLabel
    ...
    Width = 238
    Height = 18
    Caption = 'Rela'#231#227'o Faturamento x Recebimentos.'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWhite
    Font.Height = -15
    Font.Name = 'Calibri'
  ...
  object tNotas: TMSQuery
    ...
    SQL.Strings = (
      'select Pedido   = Pedido_Representante'
      '      ,Nota     = Numero'
      '      ,Data     = Data_Emissao'
      
        '      ,Volume   = round((select sum(Quantidade) from NotasItens ' +
        'ni where ni.Nota = nf.Numero and ni.data = nf.Data_Emissao), 3)'
      '      ,Faturado = round(isnull(nf.Valor_TotalNota, 0), 4)'
      
        '      ,Recebido = isnull((select sum(Valor) from PagarReceberBai' +
        'xas where Numero in(select Numero from PagarReceber pr where pr.' +
        'Fiscal = cast(nf.Numero as varchar(15)) and pr.Data_Vencimento =' +
        ' nf.Data_Emissao and Tipo_Nota = '#39'SD'#39' )), 0)'
      '      ,Representante'
      
        '      ,Cliente_CNPJ = (select CNPJ from Clientes cli where cli.C' +
        'odigo = nf. Cliente_Codigo)'
      
        '      ,Cliente_Nome = (select Nome from Clientes cli where cli.C' +
        'odigo = nf. Cliente_Codigo)'
      'from NotasFiscais nf'
      'where isnull(Pedido_Representante, '#39#39') <> '#39#39
      'and nf.Data_Emissao between '#39'03/01/2024'#39' and '#39'03/31/2024'#39
      'and Saida_Entrada = 1'
      'and isnull(nf.Cancelada, 0) = 0'
      'and isnull(nf.Nfe_Denegada, 0) = 0'
      'and isnull(Complementar, 0) = 0'
      
        'order by Data, Nota, cast(substring(Pedido_Representante, 2, 10)' +
        ' as int)')
    FetchRows = 1
  ...
  object rFatRec: TppReport
    AutoStop = False
    DataPipeline = pNotas
    NoDataBehaviors = [ndMessageDialog, ndBlankPage]
    PrinterSetup.BinName = 'Default'
    PrinterSetup.DocumentName = 'Rela'#231#227'o Faturamento x Recebimentos'
    ...
    object ppHeaderBand1: TppHeaderBand
      ...
      object lTitulo: TppLabel
        ...
        AutoSize = False
        Border.mmPadding = 0
        Caption = 'Faturamento x Recebimentos'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 9
  ...
  object tClientes: TMSQuery
    Connection = Dados.Banco_Empresas
    SQL.Strings = (
      'select Codigo, CNPJ, Nome FROM Clientes')
    FetchRows = 1
    RefreshOptions = [roAfterInsert, roAfterUpdate, roBeforeEdit]
    Active = True
    Left = 322
