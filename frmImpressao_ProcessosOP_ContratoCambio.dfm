object Impressao_ProcessosOP_ContratoCambio: TImpressao_ProcessosOP_ContratoCambio
  Left = 498
  Top = 281
  BorderStyle = bsDialog
  Caption = 'Impressao_ProcessosOP_ContratoCambio'
  ClientHeight = 335
  ClientWidth = 559
  Color = clBtnFace
  ...
  object cSituacao: TRadioGroup
    ...
    Width = 543
    Height = 45
    Caption = 'Situa'#231#227'o do Processo (Quanto ao Fechamento)'
    Columns = 4
    ItemIndex = 0
    Items.Strings = (
      'Abertos'
  ...
  object cContrato: TRxDBLookupCombo
    Left = 106
    Top = 153
    Width = 149
    Height = 21
    ...
    Font.Style = [fsBold]
    LookupField = 'Numero'
    LookupDisplay = 'Numero; Nome'
    LookupSource = dstListaContratos
    ParentFont = False
    TabOrder = 15
  end
  object cTotalContrato: TCheckBox
    Left = 8
    Top = 233
    Width = 140
    Height = 17
    Caption = 'Totalizar por contrato'
    TabOrder = 16
  end
  object tContratos: TMSQuery
    ...
    SQL.Strings = (
      'SELECT CI.Processo,'
        ...
        '       (SELECT Nome          FROM Fornecedores        WHERE(Codi' +
        'go = CC.Exportador)) AS Exportador,'
      
        '       (SELECT Nome          FROM Clientes            WHERE(Codi' +
        'go = (SELECT Cliente FROM ProcessosDocumentos WHERE(ProcessosDoc' +
        'umentos.Processo = CC.Processo)))) AS Cliente,'
      
        '       (SELECT Numero_Fatura FROM ProcessosDocumentos WHERE(Proc' +
        'essosDocumentos.Processo = CC.Processo)) AS Numero_Fatura,'
      
        '       ISNULL((SELECT Valor_FaturaME FROM ProcessosDocumentos WH' +
        'ERE(ProcessosDocumentos.Processo = CC.Processo)),0) AS Valor_Fat' +
        'uraME,'
      
        '       Vencimento_Cambio = (SELECT ISNULL(Vencimento1,0)+ISNULL(' +
        'Vencimento2,0)+ISNULL(Vencimento3,0) FROM CondicaoCambial WHERE(' +
        'Codigo = CC.Condicao_Cambio)),'
      '       CC.Numero AS Contrato_Cambio,'
      
        '       Data_Pagamento = ISNULL((SELECT MAX(Data) FROM PagarReceb' +
        'erBaixas PRB WHERE(PRB.Numero = CI.Financeiro_Lancamento)), NULL' +
        '),'
      '       CI.Taxa_Cambial,'
      '       CI.Valor_ME,'
      '       (CI.Valor_ME * CI.Taxa_Cambial) AS Valor_Real,'
      
        '       (SELECT Descricao FROM Cybersoft_Cadastros.dbo.Recursos W' +
        'HERE(Codigo = CC.Recursos)) AS Origem_Recurso,'
      
        '       Banco_Exportador = (SELECT Banco_Nome  FROM Fornecedores ' +
        'WHERE(Codigo = (SELECT Exportador FROM ContratoCambio WHERE(Nume' +
        'ro = CI.Contrato)) )),'
      
        '       Conta_Exportador = (SELECT Banco_Conta FROM Fornecedores ' +
        'WHERE(Codigo = (SELECT Exportador FROM ContratoCambio WHERE(Nume' +
        'ro = CI.Contrato)) )),'
      
        '       Swift            = (SELECT Swift_Code  FROM Fornecedores ' +
        'WHERE(Codigo = (SELECT Exportador FROM ContratoCambio WHERE(Nume' +
        'ro = CI.Contrato)) )),'
      
        '       IBAN             = (SELECT IBAN        FROM Fornecedores ' +
        'WHERE(Codigo = (SELECT Exportador FROM ContratoCambio WHERE(Nume' +
        'ro = CI.Contrato)) ))'
      'FROM   ContratoCambio CC, ContratoCambioItens CI'
      'WHERE  (CI.Contrato = CC.Numero)'
      '')
    Left = 232
    Top = 8
  end
  object rContratos: TppReport
    AutoStop = False
    DataPipeline = pContratos
    PrinterSetup.BinName = 'Default'
    PrinterSetup.DocumentName = 'Contratos de C'#226'mbio'
    ...
    Template.FileName = 'C:\Sistemas\Faturamento\Codigo Fonte\Relatorios\Contratos.rtm'
  ...
  object pContratos: TppDBPipeline
    ...
    object pContratosppField9: TppField
    ...
    object pContratosppField10: TppField
      FieldAlias = 'Valor_ME'
      FieldName = 'Valor_ME'
      FieldLength = 0
      DataType = dtNotKnown
    ...
    object pContratosppField11: TppField
    ...
    object pContratosppField12: TppField
    ...
    object pContratosppField13: TppField
    ...
    object pContratosppField14: TppField
      FieldAlias = 'Conta_Exportador'
    ...
    object pContratosppField15: TppField
      FieldAlias = 'Swift'
      FieldName = 'Swift'
      FieldLength = 0
      DataType = dtNotKnown
    ...
    object pContratosppField16: TppField
      FieldAlias = 'IBAN'
      FieldName = 'IBAN'
      FieldLength = 0
      DataType = dtNotKnown
  ...
  object tListaContratos: TMSQuery
    Connection = Dados.Banco_Empresas
    SQL.Strings = (
      'SELECT Numero,'
    ...
    Left = 296
    Top = 8
    object tListaContratosNumero: TStringField
      FieldName = 'Numero'
      Size = 15
    end
    object tListaContratosNome: TStringField
      FieldName = 'Nome'
      ReadOnly = True
      Size = 60
    end
  end
  object dstListaContratos: TDataSource
    DataSet = tListaContratos
    Left = 326
    Top = 8
  end
end
