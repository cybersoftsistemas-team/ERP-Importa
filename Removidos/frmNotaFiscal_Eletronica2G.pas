...
uses
    ...
    raCodMod, Grids, ppDesignLayer, RxToolEdit, IdIOHandlerSocket, IdBaseComponent, idText, IdComponent, IdMessageClient, IdSMTPBase, ppModule, ppStrtch, ppCache, ppDB, ppComm, ppRelatv, MemDS;
...
begin
      ...
      // Carregando configuração de dado e hora de entra/saida.
      aINI                         := TIniFile.Create(ExtractFilePath(Application.ExeName)+'ImportaRelatorios.ini');
      ...
      With Dados, dmFiscal do begin
           with Empresas do begin
                ...
                mCompFinan      := fieldbyname('Banco_Dados').AsString+'.dbo.';
           end;
       ...
       With Dados, dmFiscal do begin
            ...
            For mSel := 0 to Grade.SelectedRows.Count-1 do begin
                ...
                If mQtdErros = 0 then begin
                   ...
                   // Enviando o XML da nota fiscal para o cliente.
                   If TipoNotaEmail_NFE.AsBoolean = true then begin
                      //If (Pedidos.FieldByName('Saida_Entrada').AsInteger = 1) and (ide_tpEmis = 1) and (Clientes.FieldByName('Enviar_Email').AsBoolean = false) then begin
                      If (Pedidos.FieldByName('Saida_Entrada').AsInteger = 1) and (ide_tpEmis in[1, 6, 7]) and (Clientes.FieldByName('Enviar_Email').AsBoolean = false) then begin
                         RegistraPasso('8.0');
                         cMensagens.Lines.Add('    8. Enviando o "XML" e o "PDF" do DANFE da NF-e para o(s) E-Mail(s): ');
            ...
            // Bloqueia os pedidos que estão aguardando resposta do web service.
            tAguardando.SQL.Clear;
            tAguardando.SQL.Add('SELECT COUNT(*) AS Qtde FROM Pedidos WHERE(Aguardando = 1)');
            tAguardando.Open;
      ...
      With Dados, dmFiscal, dmContab do Begin
           ...
           While not PedidosItens.Eof do begin
                 ...
                 NotasItens.Append;
                            ...
                            NotasItensBeneficio_Fiscal.AsString   := PedidosItensBeneficio_Fiscal.AsString;
           ...
           if not NotasNFe_Denegada.AsBoolean then begin
              if PedidosDIFAL_Valor.Value > 0 then begin
                 ...
                 tProvisao.SQL.Clear;
                 tProvisao.SQL.Add('SELECT MAX(Numero) AS Numero FROM '+mCompFinan+'PagarReceber');
                 tProvisao.Open;
                 ...
                 GNRE.Append;
                      ...
                      GNREFinanceiro_Numero.Value         := tProvisaoNumero.Value + 1;
                 GNRE.Post;

                 // Contas a pagar GNRE.
                 with PagarReceber do begin
                      SQL.Clear;
                      SQL.Add('select * from '+mCompFinan+'PagarReceber where Data_Documento = :pData');
           ...
           If (PedidosSaida_Entrada.Value = 1) and (PedidosComplementar.AsBoolean = False) and (mTipoEnvio <> 'ENVIO EPEC') then begin
              Fatura.SQL.Clear;
              ...
              If (PedidosDuplicata.RecordCount > 0) then begin
                 ...
                 ClassificacaoFinanceira.ParamByName('pClassificacao').AsString := TipoNotaClassificacao_Saida.AsString;
                 ...
                 While not PedidosDuplicata.Eof do begin
                       ...
                       if not NotasNFe_Denegada.AsBoolean then begin
                          ...
                          If (TipoNotaGerar_Financeiro.Value = True) and (Trim(TipoNotaClassificacao_Saida.AsString) <> '')  then begin
                             ...
                             PagarReceber.SQL.Add('select * from '+mCompFinan+'PagarReceber where Data_Documento = :pData');
                             PagarReceber.ParamByName('pData').AsDate := NotasData_Emissao.Value;
                             PagarReceber.Open;

                             PagarReceber.Append;
                                          PagarReceberNumero.Value           := tProvisaoNumero.Value + 1;
                                          If PedidosSaida_Entrada.Value = 1 then PagarReceberTipo.Value := 'R' else PagarReceberTipo.Value := 'P';
                                          ...
                                          PagarReceberCliente.Value          := PedidosCliente_Codigo.Value;
                                          PagarReceberDocumento.Value        := TipoNotaTipo_Documento.Value;
                                          If ConfiguracaoSequencial_Duplicata.AsInteger = 0 then
                                             PagarReceberNumero_Documento.Value := FormatMaskText('999999999-A;0; ',DuplicatasDuplicata.Value)
                                          else
                                             PagarReceberNumero_Documento.Value := FormatMaskText('999999999-A;0; ',DuplicatasNumero.Value);
                                          PagarReceberProcesso.Value         := NotasProcesso.Value;
                                          PagarReceberCustoConta.Value       := ClassificacaoFinanceiraCusto.Value;
                                          PagarReceberCusto_Seletivo.Value   := ClassificacaoFinanceiraCusto_Seletivo.Value;
                                          PagarReceberCusto_Entrada.Value    := ClassificacaoFinanceiraCusto_Entrada.Value;
                                          ...
                                          If NotasSaida_Entrada.Value = 0 then
                                             ...
                                             PagarReceberTipo_Nota.Value := 'SD';
                                          PagarReceberEmpresa.Value             := Menu_Principal.mEmpresa;
                                          PagarReceberAdiantamento_Numero.Value := iif(PedidosDuplicataAdiantamento.asboolean, PedidosCliente_Codigo.Value, 0);
                             PagarReceber.Post;
                             tProvisao.Close;

                             // Contas a Pagar (Pagamento de comissão a Representantes).
                             If (NotasRepresentante.Value > 0) and (NotasRepresentante_Comissao.Value > 0) and (ConfiguracaoClassificacao_Pagamento.AsString <> '') then begin
                                ClassificacaoFinanceira.SQL.Clear;
                                ClassificacaoFinanceira.SQL.Add('SELECT * FROM ClassificacaoFinanceira WHERE (Codigo = :pClassificacao)');
                                ClassificacaoFinanceira.ParamByName('pClassificacao').AsString := ConfiguracaoClassificacao_Pagamento.AsString;
                                ClassificacaoFinanceira.Open;
                 ...
                 if not NotasNFe_Denegada.AsBoolean then begin
                    If (TipoNotaGerar_Financeiro.AsBoolean = True) then begin
                       ...
                       If (ConfiguracaoGerar_DuplImpostos.AsBoolean = false) then begin
                          mPercentual := 0;
                       ...
                       If ConfiguracaoGerar_DuplImpostos.AsBoolean = true then begin
                          PagarReceber.SQL.Clear;
                          PagarReceber.SQL.Add('select * from '+mCompFinan+'PagarReceber where Data_Documento = :pData');
                          PagarReceber.ParamByName('pData').AsDate := NotasData_Emissao.Value;
                          PagarReceber.Open;
              ...
              If (PedidosSaida_Entrada.Value = 1) and (PedidosComplementar.AsBoolean = true) and (mTipoEnvio <> 'ENVIO EPEC') then begin
                 If (Trim(TipoNotaClassificacao_Saida.AsString) <> '') then begin
                    ...
                    PagarReceber.Append;
                                 ...
                                 PagarReceberValor_Total.Value      := RoundTo(PagarReceberValor_Parcela.Value, -2);
                                 PagarReceberValor_Operacao.Value   := RoundTo(PagarReceberValor_Parcela.Value, -2);
                                 PagarReceberCliente.Value          := NotasCliente_Codigo.Value;
                                 PagarReceberDocumento.Value        := TipoNotaTipo_Documento.Value;
                                 PagarReceberNumero_Documento.Value := NotasNumero.AsString;
                                 PagarReceberProcesso.Value         := NotasProcesso.Value;
                                 PagarReceberCustoConta.Value       := ClassificacaoFinanceiraCusto.Value;
...
begin
       ...
       With Dados, dmFiscal do begin
            ...
            {
            if PedidosSaida_Entrada.asinteger = 0 then begin
               DetPag := Util.detPag202006(PedidosTipo_Pagamento.AsString                                                    // 01 informar o indicador da forma de pagamento:0 - pagamento à vista / 1 - pagamento à prazo.
                                         ...
                                         , '2'                                                                               // 05 Tipo de Integração do processo de pagamento com o sistema de automação da empresa.
               ...
               DetPag := Util.detPag2023004(PedidosTipo_Pagamento.AsString                                                   // 01 informar o indicador da forma de pagamento:0 - pagamento à vista / 1 - pagamento à prazo.
                                           ,Poezero(2, PedidosForma_Pagamento.AsInteger)                                     // 02 informar o Meio de pagamento.
                                           ,PedidosDescricao_Forma.asstring                                                  // 03 informar a descrição do meio de pagamento quando tPag for informado com 99.
                                           ,iif(PedidosForma_Pagamento.AsInteger <> 90, PedidosValor_TotalNota.AsFloat, 0)   // 04 informar o valor do Pagamento.
                                           ,'2'                                                                              // 05 Tipo de Integração do processo de pagamento com o sistema de automação da empresa.
            ...
            totICMS := Util.totalICMSNT2023001(PedidosBCICMS.AsCurrency,                                                       //01 somatório da BC do ICMS (vBC) informado nos itens.
                                               ...
                                               0,                                                                              //22 informar o somatório do Valor do IPI devolvido. Deve ser informado quando preenchido o Grupo Tributos Devolvidos na emissão de nota finNFe=4 (devolução) nas operações com não contribuintes do IPI.
                                               0,                                                                              //23 somatório do Valor do IPI devolvido.
            ...
            *)
            ...
            RespTec := util.infRespTec('39940892000146', 'EDER/ELISMAR', 'elismar@cybersoftsistemas.com.br', '2730267408', '', '', '');
...
begin
      With Dados, dmFiscal do Begin
           ...
           Adicoes.Locate('Codigo_Mercadoria', PedidosItensCodigo_Mercadoria.Value, [loCaseInsensitive]);
           ...
           _produto := Util.produto400(mCodigoProd,                                                           // 01.informar o código do produto ou serviço.
           ...
           if (PedidosItensCodigoTrib_TabB.value <> '15') and (PedidosItensCodigoTrib_TabB.value <> '61') then begin
              if (PedidosItensCodigoTrib_TabB.value <> '51') then begin
                  if (PedidosItensCodigoTrib_TabB.value <> '60') and (PedidosItensCodigoTrib_TabB.value <> '500') then begin
                     _ICMS := Util.icmsNT2020005(PedidosItensCodigoTrib_TabA.Value,                                              // Origem da mercadoria.
               ...
               if PedidosItensCodigoTrib_TabB.value = '61' then begin
                  _ICMS := Util.icms61(PedidosItensCodigoTrib_TabA.Value             // 1-Origem da mercadoria
                                      ...
                                      ,ProdutosPercentual_ICMSMono.asfloat           // 3-Alíquota ad rem do ICMS, estabelecida na legislação para o produto.
           ...
           if PedidosItensDIFAL_Valor.Value > 0 then begin
              _ICMSUFDest := Util.ICMSUFDest400(PedidosItensValor_BCDIFAL.Value,        // 01. Valor da Base de Cálculo do ICMS na UF de destino.
                                                PedidosItensValor_BCICMSOper.Value,     // 02. Informar o Valor da Base de Cálculo do FCP na UF Destino
                                                PedidosAliquota_FCP.Value,              // 03. Percentual adicional inserido na alíquota interna da UF de destino,(FCP) naquela UF.
           ...
           // Monta as TAGS de impostos e detalhamento dos produtos.
           _Imposto := Util.impostoNT2015003(PedidosItensTotal_Impostos.AsCurrency, _ICMS, _IPI, _II, _PIS, '', _COFINS, '', '', _ICMSUFDest);
...
begin
      ...
      NFeAssinada := Util.EnviaNfe2G(SiglaWS
                                    ,NFe
                                    ,NomeCertificado
      ...
      If cStat <> 103 then begin
         // Fanzendo a consistencia do XML.
         With Dados, dmFiscal do begin
          ...
          if mEnviar = true then begin
             ...
             with EmailMsg do begin
                  ...
                  if Tipo = 'CLIENTE' then begin
                     ...
                     ccList.EMailAddresses     := Trim(EmpresasEmail_Copia.AsString);

                     // Copias para os emails do cliente.
                     if Trim(ClientesEmail_Copia.AsString) <> '' then begin
                        ccList.EMailAddresses := Trim(EmpresasEmail_Copia.AsString)+';'+Trim(ClientesEmail_Copia.AsString);
                     end;

                     // Copias para os emails do Transportador.
                     if Fornecedores.Locate('Codigo', PedidosTransportador_Codigo.Value , [loCaseInsensitive]) = true then begin
                        if (Trim(FornecedoresEmail.AsString) <> '') and (FornecedoresEnviar_Email.AsBoolean = true) then begin
                           ccList.EMailAddresses := Trim(EmpresasEmail_Copia.AsString)+';'+Trim(ClientesEmail_Copia.AsString)+';'+Trim(FornecedoresEmail.AsString);
                        end;
                     end;
                  end else begin
             ...
             if SMTP.Connected then begin
                ...
                if Tipo = 'CLIENTE' then
                   cMensagens.Lines.Add('     Notas Fiscal eletrônica enviada para o cliente...')
                else
                   cMensagens.Lines.Add('     Notas Fiscal eletrônica enviada para o E-Mail fixo...'+ConfiguracaoEmail_NFE.AsString);
             end else begin
      ...
      With Dados, dmFiscal do begin
           ...
           While not PedidosItens.Eof do begin
                 ...
                 If (PedidosImportacao.AsBoolean) and (PedidosSaida_Entrada.AsInteger = 0) and (Trim(PedidosItensProcesso.AsString) <> '') and (ProcessosDOCVia_Transporte.AsInteger =  1) then begin
                    ...
                    tDespesas.SQL.Clear;
                    tDespesas.SQL.Add('select * from '+mCompFinan+'PagarReceber where Processo = :pProcesso and Classificacao = :pClassificacao');
...
begin
      with Dados do begin
           Empresas.Edit;
                    EmpresasEmissaoNFE_Bloqueada.Value := Tipo;
                    if Tipo then
                       EmpresasEmissaoNFE_BloqueadaUsuario.Value := Menu_Principal.mUsuarioNome + ' ('+NomeComputador+')'
                    else
