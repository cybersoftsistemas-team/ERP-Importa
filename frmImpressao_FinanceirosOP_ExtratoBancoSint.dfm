object Impressao_FinanceirosOP_ExtratoBancoSint: TImpressao_FinanceirosOP_ExtratoBancoSint
  Left = 572
  Top = 321
  BorderStyle = bsDialog
  Caption = 'Impressao_FinanceirosOP_ExtratoBancoSint'
  ClientHeight = 244
  ClientWidth = 545
  Color = clBtnFace
  ...
  object RxLabel2: TRxLabel
    ...
    Width = 176
    Height = 18
    Caption = 'Extrato banc'#225'rio (Sint'#233'tico).'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWhite
    Font.Height = -15
    Font.Name = 'Calibri'
  ...
  object rF02: TppReport
    AutoStop = False
    DataPipeline = pPagarReceber
    NoDataBehaviors = [ndBlankReport]
    PassSetting = psTwoPass
    PrinterSetup.BinName = 'Default'
    PrinterSetup.DocumentName = 'Extrato banc'#225'rio Sint'#233'tico'
    ...
    Template.FileName = 
      'C:\Sistemas\Faturamento\Codigo Fonte\Relatorios\FinanceirosExtra' +
      'toBancario.rtm'
    ...
    object bDetalhe: TppDetailBand
      AfterGenerate = bDetalheAfterGenerate
      Background1.Brush.Color = 16772332
      Background1.Gradient.EndColor = clNone
      Background1.Gradient.StartColor = clNone
      Background2.Gradient.StartColor = clNone
      Background2.Gradient.Style = gsVertical
      Border.mmPadding = 0
      mmBottomOffset = 0
      mmHeight = 6085
      mmPrintPosition = 0
  ...
  object tPagarReceber: TMSQuery
    ...
    SQL.Strings = (
      ...
      '      ,Agencia'
        ...
        '      ,Saldo_Ant = ISNULL(Saldo + (SELECT ISNULL(SUM(CASE WHEN T' +
        'ipo = '#39'R'#39' THEN Valor ELSE Valor *-1 END), 0) FROM PagarReceberBa' +
        'ixas PB WHERE PB.Banco_Conta = BC.Conta AND Data < '#39'01/01/2017'#39')' +
        ', 0)'
      
        '      ,Debito    = (SELECT ISNULL(SUM(Valor), 0) FROM PagarReceb' +
        'erBaixas PB WHERE PB.Banco_Conta = BC.Conta AND Data BETWEEN '#39'01' +
        '/01/2017'#39' AND '#39'31/01/2017'#39' AND Tipo = '#39'P'#39') *-1'
      
        '      ,Credito   = (SELECT ISNULL(SUM(Valor), 0) FROM PagarReceb' +
        'erBaixas PB WHERE PB.Banco_Conta = BC.Conta AND Data BETWEEN '#39'01' +
        '/01/2017'#39' AND '#39'31/01/2017'#39' AND Tipo = '#39'R'#39')'
      
        '      ,Saldo_Atu = ISNULL(Saldo + (SELECT ISNULL(SUM(CASE WHEN T' +
        'ipo = '#39'R'#39' THEN Valor ELSE Valor *-1 END), 0) FROM PagarReceberBa' +
        'ixas PB WHERE PB.Banco_Conta = BC.Conta AND Data <= '#39'31/01/2017'#39 +
        '), 0)'
      'FROM  Bancos BC'
      'WHERE ISNULL(Conta, '#39#39') <> '#39#39' '
      'ORDER BY Nome')
    FetchRows = 1
    Left = 241
    Top = 159
