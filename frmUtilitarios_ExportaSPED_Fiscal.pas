          ...
          If cInventario.ItemIndex <> 3 then begin
             If EmpresasMenu_Inventario.AsBoolean then begin
                with tFichainventario do begin
                     sql.clear;
                     sql.add('if object_id(''dbo.PegaValor'', ''FN'') is not null');
                     sql.add('   drop function dbo.PegaValor');
                     execute;

                     sql.Clear;
                     sql.Add('--------------------------------------------[ CRIA A FUNÇÃO QUE PEGA O VALOR DO PRODUTO NO ARMAZEM ]------------------------------------------------');
                     sql.add('create function PegaValor(@Cod int, @Arm varchar(15), @Data date)');
                     SQL.Add('returns money');
                     SQL.Add('begin');
                     SQL.Add('    declare @Retorno money');
                     SQL.Add('    set @Retorno = isnull((select isnull(fi.Unitario_Entrada, 0)+isnull(fi.Unitario_Saida, 0)');
                     SQL.Add('                    from FichaInventario fi');
                     SQL.Add('                    where fi.Codigo = @Cod');
                     SQL.Add('                    and fi.Estoque = @Arm');
                     SQL.Add('                    and fi.Data <= @Data');
                     sql.add('                    and fi.Item = (select max(fi2.Item) from FichaInventario fi2 where fi2.Codigo = @Cod and fi2.Estoque = @Arm and fi2.Data <= @Data)), 0)');
                     ...
                     SQL.Add('      ,FI1.ES');
                     SQL.Add('      ,FI1.Destinatario_Codigo');
                     sql.add('      ,Qtde_Saldo = (select sum(isnull(fi2.Qtde_Saida, 0)-isnull(fi2.Qtde_Entrada, 0)) from FichaInventario fi2 where fi2.Codigo = fi1.Codigo and fi2.Estoque = ''1-ARMAZEM'' and fi2.Data <= :pData)');
                     SQL.Add('      ,Unitario_Saldo = case when fi1.Unitario_Saldo > 0 then ');
                     SQL.Add('                             fi1.Unitario_Saldo');
                     SQL.Add('                        else');
                     sql.add('                             dbo.PegaValor(fi1.Codigo, ''1-ARMAZEM'', :pData)');
                     SQL.Add('                        end');
                     ...
                     SQL.Add('       ,FI1.ES');
                     SQL.Add('       ,FI1.Destinatario_Codigo');
                     sql.add('       ,Qtde_Saldo = (select sum(isnull(fi2.Qtde_Saida, 0)-isnull(fi2.Qtde_Entrada, 0)) from FichaInventario fi2 where fi2.Codigo = fi1.Codigo and fi2.Estoque = ''2-TERCEIROS'' and fi2.Data <= :pData)');
                     SQL.Add('       ,Unitario_Saldo = case when fi1.Unitario_Saldo > 0 then ');
                     SQL.Add('                              fi1.Unitario_Saldo');
                     SQL.Add('                         else');
                     sql.add('                              dbo.PegaValor(fi1.Codigo, ''2-TERCEIROS'', :pData)');
                     SQL.Add('                         end');
                     SQL.Add('       ,Total_Saldo = 0');
                     SQL.Add('       ,Tipo = CASE WHEN Emissor = ''P'' AND ES = ''E'' THEN ''F'' ');
                     SQL.Add('                    WHEN Emissor = ''P'' AND ES = ''S'' THEN ''C'' ');
...
//                     SQL.Add('       ,Total_Saldo = (FI1.Unitario_Entrada + fi1.Unitario_Saida) * (select sum(fi2.Qtde_Saida-fi2.Qtde_Entrada) from FichaInventario fi2 where fi2.Codigo = fi1.Codigo and fi2.Estoque = ''1-ARMAZEM'' and fi2.Data <= :pData)');
                     SQL.Add('       ,Unitario_Saldo = case when Unitario_Saldo > 0 then');
                     SQL.Add('                              Unitario_Saldo');
                     SQL.Add('                         else');
                     SQL.Add('                             (select isnull(fi.Unitario_Entrada, 0) + isnull(fi.Unitario_Saida, 0)');
                     SQL.Add('                              from FichaInventario fi');
                     SQL.Add('                              where fi.Codigo = fi1.Codigo');
                     SQL.Add('                              and fi.Estoque = ''1-ARMAZEM'' ');
                     SQL.Add('                              and fi.Data <= :pData');
                     SQL.Add('                              and fi.Item = (select max(fi2.Item) from FichaInventario fi2 where fi2.Codigo = fi.Codigo and fi2.Estoque = ''1-ARMAZEM'' and fi2.Data <= :pData))');
                     SQL.Add('                         end');
                     sql.add('       ,Total_Saldo = 0');
                     SQL.Add('FROM   FichaInventario FI1');
                     SQL.Add('WHERE  FI1.Estoque = ''1-ARMAZEM'' ');
...
//                     SQL.Add('       ,Total_Saldo = (FI1.Unitario_Entrada + fi1.Unitario_Saida) * (select sum(fi2.Qtde_Saida-fi2.Qtde_Entrada) from FichaInventario fi2 where fi2.Codigo = fi1.Codigo and fi2.Estoque = ''1-ARMAZEM'' and fi2.Data <= :pData)');
                     ...
                     SQL.Add('                              from FichaInventario fi');
                     SQL.Add('                              where fi.Codigo = fi1.Codigo');
                     SQL.Add('                              and fi.Estoque = ''2-TERCEIROS'' ');
                     SQL.Add('                              and fi.Data <= :pData');
                     SQL.Add('                              and fi.Item = (select max(fi2.Item) from FichaInventario fi2 where fi2.Codigo = fi.Codigo and fi2.Estoque = ''2-TERCEIROS'' and fi2.Data <= :pData))');
                     SQL.Add('                         end');
                     sql.add('       ,Total_Saldo = 0');
          ...
          If cEstoque.ItemIndex <> 3 then begin
             If EmpresasMenu_Inventario.AsBoolean then begin
                with tFichaEstoque do begin
                     ...
                     SQL.Add('    declare @Retorno money');
                     SQL.Add('    set @Retorno = isnull((select isnull(fi.Unitario_Entrada, 0)+isnull(fi.Unitario_Saida, 0)');
                     SQL.Add('                           from FichaEstoque fi');
                     SQL.Add('                           where fi.Codigo = @Cod');
                     SQL.Add('                           and fi.Estoque = @Arm');
                     SQL.Add('                           and fi.Data <= @Data');
                     sql.add('                           and fi.Item = (select max(fi2.Item) from FichaEstoque fi2 where fi2.Codigo = @Cod and fi2.Estoque = @Arm and fi2.Data <= @Data)), 0)');
                     SQL.Add('    return(@Retorno)');
                     SQL.Add('end');
                     execute;
                     ...
                     SQL.Add('       ,FI1.ES');
                     SQL.Add('       ,FI1.Destinatario_Codigo');
                     sql.add('      ,Qtde_Saldo = (select sum(isnull(fi2.Qtde_Saida, 0)-isnull(fi2.Qtde_Entrada, 0)) from FichaEstoque fi2 where fi2.Codigo = fi1.Codigo and fi2.Estoque = ''1-ARMAZEM'' and fi2.Data <= :pData)');
                     SQL.Add('      ,Unitario_Saldo = case when fi1.Unitario_Saldo > 0 then ');
                     SQL.Add('                             fi1.Unitario_Saldo');
                     SQL.Add('                        else');
                     sql.add('                             dbo.PegaValor(fi1.Codigo, ''1-ARMAZEM'', :pData)');
